...
// make the display for all submission history
    ...
    if($scope.workorder.timeSubmitted){
      // Tech Submission
      _.map($scope.users, function(user){
        if(user.username === $scope.workorder.techId){
          var techSubmission = ClassSubmission();
          techSubmission.type = "Submission";
          techSubmission.firstname = user.firstName;
          techSubmission.lastname = user.lastName;
      ...
      _.map($scope.users, function(user){
        if($scope.workorder.timeApproved){
          if(user.username === $scope.workorder.approvedBy){
            var managerSubmission = ClassSubmission();
            managerSubmission.type = "Reviewed";
            managerSubmission.firstname = user.firstName;
            managerSubmission.lastname = user.lastName;
      ...
      _.map($scope.users, function(user){
        if($scope.workorder.timeSynced){
          if(user.username === $scope.workorder.syncedBy){
            var adminSubmission = ClassSubmission();
            adminSubmission.type = "Synced";
            adminSubmission.firstname = user.firstName;
            adminSubmission.lastname = user.lastName;
...
//-----------------------------------------------------------------------
    ...
    $scope.submit = function () {
      ...
      $scope.submitting = true;
      $scope.approvedBy = $scope.me.username;
