...
const define = (agenda) => {
    agenda.define('WednesdayManHoursTotalEmail', {concurrency: 1}, (job, done) => {
        ...
        return SaturdayToWednesdayTotalsEmail()
            .then(() => {
            log.trace('Finished WednesdayManHoursTotalEmail Task');
                job.remove((err) => {
                    if (err) {
                        log.debug({error: err},
                            'Error Saturday - Wednesday this week, on remove');
                    }
                    define(agenda);
                    agenda.schedule('wednesday at 17:00',
            ...
            .catch((err) => {
            log.error({error: err},
                `Error while trying to email Wednesdays total times: ${err}`);
            log.debug({error: err},
                'Error while trying to email Saturday - Wednesday, on execution');
                // remove job form db and reschedule
                job.remove((err) => {
                    if (err) {
                        log.debug({error: err},
                            'Error Saturday - Wednesday this week, on execution, remove');
                    }
                    define(agenda);
                    agenda.schedule('wednesday at 17:00',
                        'WednesdayManHoursTotalEmail');
...
const start = (agenda) => {
    ...
    agenda.on('ready', () => {
        ...
        // agenda.schedule('tuesday at 21:28', 'WednesdayManHoursTotalEmail');
    });
    // ------------------------------------------------------------------------
};
