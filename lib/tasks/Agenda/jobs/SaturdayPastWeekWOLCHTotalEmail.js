        ...
        return SaturdayFridayTotalEmail()
            ...
            log.trace('Finished SaturdayPastWeekWOLCHTotalEmail Task');
                ...
                job.remove((err) => {
                    if (err) {
                        log.debug({error: err},
                            'Error Saturday Past week email, on remove');
                    }
                    define(agenda);
                    agenda.schedule('saturday at 15:00',
            ...
            .catch((err) => {
            log.error({error: err},
                `Error while trying to email Saturday total times: ${err}`);
            log.debug({error: err},
                'Error while trying ot email Saturday total, on execution');
                // remove job from db and reschedule
                job.remove((err) => {
                    if (err) {
                        log.debug({error: err},
                            'Error Saturday Past week email, on execution, remove');
                    }
                    define(agenda);
                    agenda.schedule('saturday at 15:00',
                        'SaturdayPastWeekWOLCHTotalEmail');
...
const start = (agenda) => {
    ...
    agenda.on('ready', () => {
        ...
        // agenda.schedule('tuesday at 21:36',
        // 'SaturdayPastWeekWOLCHTotalEmail');
    });
    // ------------------------------------------------------------------------
};
