...
const updateSyncedFalse = () => {
    return new Promise((resolve, reject) => {
        Vendor.update({isSynced: true}, {isSynced: false}, {multi: true}, (err) => {
            if (err) return reject(err);
            resolve();
        });
    });
...
const vendorFormat = (ele) => {
    ...
    const category = ele.columns.hasOwnProperty('category') ?
                     (ele.columns.category.hasOwnProperty('name') ? ele.columns.category.name : '') : '';
    const email = ele.columns.hasOwnProperty('email') ? ele.columns.email : '';
    const contact = ele.columns.hasOwnProperty('contact') ?
                    (ele.columns.contact.hasOwnProperty('name') ? ele.columns.contact.name : '') : '';
...
const setNotSyncedToInactive = () => {
    return new Promise((resolve, reject) => {
        Vendor.update({isSynced: false}, {activeStatus: true}, {multi: true},
            (err) => {
                if (err) return reject(err);
                resolve();
            });
