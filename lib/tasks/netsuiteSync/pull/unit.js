...
const unitSearchUrl1 = {
    url:
        "https://rest.na1.netsuite.com/app/site/hosting/restlet.nl?script=248&deploy=1&recordtype=customrecord_ncfar_asset&id=91",
    id: 91,
};
...
const unitFormat = (ele) => {
    return new Promise((resolve, reject) => {
        ...
        if (ele.columns.hasOwnProperty("custrecord_sku")) {
            sku = ele.columns.custrecord_sku.name.toUpperCase();
        }
        ...
        getAsyncUnitInfo(engine, frame, county, state, nsUserId)
            .then((res) => {
                resolve({
                    ...
                    sku: sku,
                    locationName: ele.columns.hasOwnProperty(
                        "custrecord_leasename"
                    )
                        ? ele.columns.custrecord_leasename.toUpperCase()
                        : "",
                    legalDescription: ele.columns.hasOwnProperty(
                    ...
                    pmCycle: ele.columns.hasOwnProperty(
                        "custrecord_pmcycletime"
                    )
                        ? ele.columns.custrecord_pmcycletime.name
                        : "",
                    nextPmDate: ele.columns.hasOwnProperty(
