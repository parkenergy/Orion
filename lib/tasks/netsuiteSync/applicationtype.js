...
const addChangeLog = (docs, applicationTypes) => {
  return new Promise((resolve, reject) => {
    ...
    applicationTypes.forEach((applicationType) => {
      let found = false;
      docs.forEach((doc) => {
        if (doc.netsuiteId === applicationType.netsuiteId && doc.netsuiteId !== null) {
          found = true;
          changes.push({diff: diff(applicationType, doc), old: applicationType.netsuiteId, newDoc: doc.netsuiteId});
        }
        if (doc.netsuiteId === null) {
          found = true;
        }
      });
      if (!found) {
        changes.push({diff: 'removed', old: applicationType.netsuiteId, newDoc: null});
      }
    });
    docs.forEach((doc) => {
      let found = false;
...
const updateSyncedFalse = () => {
  return new Promise((resolve) => {
    ApplicationType.update({isSynced: true}, {isSynced: false}, {multi: true}, () => {
      resolve();
    })
  });
};
...
const getApplicationTypes = (callback) => {
  let docs;
  let FormattedDocs;
  ...
  queryApplicationTypes(applicationTypeListUrl, options)
    ...
    .then((locations) => {
      FormattedDocs = docs.map((d) => applicationTypeFormat(d));
      FormattedApplicationTypes = locations.map((u) => formatApplicationType(u));
