...
const EmailSupervisor = (spo, start, end) => {
  return new Promise((resolve, reject) => {
    try {
...
const EmailOrionAlerts = (spo, start, end) => {
  return new Promise((resolve, reject) => {
    try {
      if (spo.SupervisorObj.email) {
        const mailer = new GmailMailer();
        mailer.transport.verify((err, success) => {
          if (err) {
            ...
            const mailerOptions = {
              from: `"Orion Alerts" <orionalerts@parkenergyservices.com>`,
              subject: `Sunday: ${start.returnStart.toLocaleDateString()} - Saturday: ${end.returnEnd.toLocaleDateString()} | WO Times | For ${spo.SupervisorObj.firstName} ${spo.SupervisorObj.lastName}`,
              to: 'mwhelan@parkenergyservices.com',
 ...
 * @constructor
 */
const EmailOut = (managers, superObj, start, end) => {
  const promises = [];
  ...
  managerKeys.forEach((manager) => {
    ...
    if (superVisorObj.SupervisorObj) {
      promises.push(EmailSupervisor(superVisorObj, start, end));
      // promises.push(EmailOrionAlerts(superVisorObj, start, end));
...
const SundaySaturdayTotalEmail = () => {
  return new Promise((resolve, reject) => {
    ...
    TH.getUsers()
      ...
      .then((obj) => EmailOut(managers, obj, SUNDAY, SATURDAY))
      .then(resolve)
      .catch(reject);
  });
};
