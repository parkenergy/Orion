...
const getUserPTOHours = (obj) => {
  return new Promise((resolve, reject) => {
    PaidTimeOff.find({
      ...
      .then((ptos) => {
        if (ptos.length === 0) {
          resolve({user: obj.user, times: obj.times, Indirect: obj.Indirect, PM: obj.PM, Corrective: obj.Corrective, TroubleCall: obj.TroubleCall, NewSet: obj.NewSet, Travel: obj.Travel, Release: obj.Release, PTO: 0});
        } else {
          const PTO = ptos.reduce((a, b) => {
            return a + b.hours;
          }, 0);
          resolve({user: obj.user, times: obj.times, Indirect: obj.Indirect, PM: obj.PM, Corrective: obj.Corrective, TroubleCall: obj.TroubleCall, NewSet: obj.NewSet, Travel: obj.Travel, Release: obj.Release, PTO})
        }
      })
      .catch(reject);
  });
...
const doesUserNeedEmail = (techObj, type) => {
  return new Promise((resolve) => {
    if (techObj.user.hasOwnProperty('hours') && (techObj.user.hours !== null && techObj.user.hours !== undefined)) {
          ...
          mailer.transport.verify((err, success) => {
            if (err) {
              ...
              try {
                ...
                const to = process.env.NODE_ENV === undefined ? 'mwhelan@parkenergyservices.com' : toEmail;
                mailOptions = {
        ...
        if (bool) {
          mailer.transport.verify((err, success) => {
            if (err) {
              ...
              let mailOptions;
              try {
                const to = process.env.NODE_ENV === undefined ? 'mwhelan@parkenergyservices.com' : 'orionalerts@parkenergyservices.com';
                mailOptions = {
                  from: `"Orion Alerts" <orionalerts@parkenergyservices.com>`,
                  subject: `Technician Time Report | ${now}`,
                  to: to,
