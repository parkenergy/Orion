            ...
            managers.forEach((manager) => {
                SuperObj[manager] = {
                    techs:         {}, // will be filled with all managers
                                       // techs + times
                    SupervisorObj: {supervisor: null, email: null}, // this
                                                                    // managers
                                                                    // obj to
                                                                    // be
                };
...
const renderTableRow = (tech, SuperObjManagerTECHS, TimeObj) => {
    ...
    <tr id="${tech}">
        ...
        <td>
          <a  href="http://orion.parkenergyservices.com/#/workorder/v2Search/${TimeObj.Saturday.times.returnStart}/${TimeObj.Wednesday.times.returnEnd}/${SuperObjManagerTECHS[tech].tech.username}">
...
const SaturdayToWednesdayTotalsEmail = () => {
    return new Promise((resolve, reject) => {
        ...
        TH.getUsers()
            ...
            // total to payroll.
            .then(() => WaterFallEmail(SuperObj, SATURDAY, WEDNESDAY, TimesObj))
            .then(resolve)
            .catch(reject);
    });
};
