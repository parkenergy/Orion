...
const addTechObjDay = (managerHash, techObj, returnObj) => {
    ...
    returnObj[techObj.user.supervisor].techs[techObj.user.username]['Total'] = returnObj[techObj.user.supervisor].techs[techObj.user.username]['Total']
        ? returnObj[techObj.user.supervisor].techs[techObj.user.username]['Total']
        : 0;
...
const NoManagerObjDay = (techObj, returnObj) => {
    ...
    returnObj['Orion'].techs[techObj.user.username]['Total'] = returnObj['Orion'].techs[techObj.user.username]['Total']
        ? returnObj['Orion'].techs[techObj.user.username]['Total']
        : 0;
...
const breakUpTechsBySupervisor = (managersHash, techObjArr) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            managers.push('Orion');
            managers.forEach((manager) => returnObj[manager] = {
                techs: {},
...
const EmailSupervisor = (spo, start, end, TimeObj) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            log.debug({error: e},
                'Error While emailing supervisor. Sunday - Thursday this week');
            return reject(e);
        }
    });
};
...
const emailTech = (spo, tech, start, end) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            log.debug({error: e},
                'Error While emailing technician. Sunday - Thursday this week');
            return reject(e);
        }
    });
};
...
const EmailSupervisorTechs = (spo, start, end) => {
    const promises = [];
    const techs = Object.keys(spo.techs); // get technicians from this
                                          // supervisor to email
    if (techs.length === 0) {
            ...
            .then((obj) => {
                ...
                return Promise.all(managerKeys.map((manager) => {
                    ...
                    if (SUPEROBJ.SupervisorObj) {
                        return EmailSupervisorTechs(SUPEROBJ, SATURDAY,
                            WEDNESDAY);
                    } else {
