...
module.exports = function (unitSchema) {
  ...
  unitSchema.statics.fetch = function (unitNumber) {
    ...
    return new Promise( (resolve, reject) => {
      ...
      this.findOne({number: unitNumber})
        ...
        .then(unit => unit[0]) //deconstruct array
        .then(resolve)
        .catch(reject);
      ...
      if (options.supervisor) {
        User.getTechsForSupervisor(options.supervisor)
          .then(techs => {
            ...
            log.warn({q, size, page, sort}, "Unit Query(w/ supervisor)");
  ...
  unitSchema.statics.getWorkOrders = function (unit) {
    return new Promise((resolve, reject) => {
