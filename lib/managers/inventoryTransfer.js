...
module.exports = function (inventoryTransferSchema) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.find({
                inventorytransferDate: {
                    $in: dataArr.reduce((acc, o) => {
                        if (o.hasOwnProperty("inventorytransferDate")) {
                            return acc.concat(
                                new Date(o.inventorytransferDate).getTime()
                            );
                        } else {
                            return acc;
                        }
                ...
                .then((foundDocs) => {
                    newArr = dataArr.map((doc) => {
                        ...
                        foundDocs.forEach((fDoc) => {
                            if (
                                new Date(
                                    fDoc.inventorytransferDate
                                ).getTime() ===
                                new Date(doc.inventorytransferDate).getTime()
                            ) {
                        ...
                        if (foundDup.found) {
                            ...
                            doc.inventorytransferDate = new Date(
                                doc.inventorytransferDate
                            ).getTime();
                ...
                .then((objects) => {
                    ...
                    objects.forEach((obj) => {
                        if (obj.found === false) {
                            promises.push(this.create(obj.obj));
                        }
                    });
                    ...
                    newArr.forEach((na) => {
                        docs.forEach((doc) => {
                            ...
                            if (
                                ...
                                new Date(na.obj.inventorytransferDate).getTime()
                            ) {
                                if (!na.found) {
                                    promises.push(
                                        new Promise((res) => res(doc))
                                    );
                                } else {
                                    promises.push(
                                        new Promise((res) => res(na.obj))
                                    );
                                }
                            }
                        });
    ...
    inventoryTransferSchema.statics.updateDoc = function (id, data) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.findByIdAndUpdate(id, data, { safe: false, new: true })
                ...
                .then(() => {
                    return axios.post(
                        "http://34.223.93.100:8080/transfers/"/*"http://inventorytransferservice.us-west-2.elasticbeanstalk.com/transfers/"*/ + docId,
                        data
                    );
                })
    ...
    inventoryTransferSchema.statics.fetch = function (id) {
        return new Promise((resolve, reject) => {
            ...
            this.findById(id).exec().then(resolve).catch(reject);
        });
    };
    ...
    inventoryTransferSchema.statics.delete = function (id) {
        return new Promise((resolve, reject) => {
            ...
            this.findOneAndRemove({ id }).exec().then(resolve).catch(reject);
        });
    };
