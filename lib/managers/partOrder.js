...
const ObjectId    = require('mongoose').Types.ObjectId,
      _           = require('lodash'),
      poDump      = require('../databaseScripts/poDump'),
...
module.exports = function(partOrderSchema) {
    ...
    //count docs
    partOrderSchema.statics.__count = function (options) {
        return new Promise((resolve, reject) => {
            const q = queryConstructor(options)
            this.find(q)
                .count()
                .then(resolve)
                .catch(reject)
    ...
    partOrderSchema.statics.poDumpReport = function (options) {
        return new Promise((resolve, reject) => {
            const q = queryConstructor(options)
    ...
    partOrderSchema.statics.partsReport = function (query){
        return new Promise((resolve, reject) => {
            this.aggregate()
                ...
                .then((res) => res.reduce((csv, report) => {
...
const queryConstructor = (options) => {
    ...
    if (options.ids) {
        if (Object.prototype.toString.call(options.ids) === '[object Array]') {
            options.ids = options.ids.map((id) => typeof id === 'string' ? ObjectId(id) : id)
            q._id = {$in: options.ids}
        } else {
            q._id = options.ids
        }
    }
