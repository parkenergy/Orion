...
module.exports = function(editHistorySchema) {
    ...
    editHistorySchema.statics.createDoc = function (data) {
        ...
        return new Promise((resolve, reject) => {
            const dataArr = [].concat(data); //Ensure data is an array, cast
                                             // single object to array

            dataArr.map(doc => {
                if (doc.workOrder && doc.user) {
                    if (typeof doc.workOrder ===
                        'string') doc.workOrder = ObjectId(doc.workOrder);
                    if (typeof doc.user === 'string') doc.user = ObjectId(
                        doc.user);
                }
            });
    ...
    editHistorySchema.statics.list = function (options) {
        ...
        return new Promise((resolve, reject) => {
            if (!options.workOrder) return reject(
                'Missing workOrder ID for lookup');
            if (typeof options.workOrder ===
                'string') options.workOrder = ObjectId(options.workOrder);
