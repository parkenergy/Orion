...
module.exports = function (workOrderSchema) {
    ...
    workOrderSchema.statics.createDoc = function (data) {
        return new Promise((resolve, reject) => {
            ...
            Promise.all([
                ...
                if (!isEmpty(objects)) {
                    insertObjects = objects.reduce((acc, cur) => {
                        if (cur.found === false) {
                            log.error(
                                `MANAGER (187) Type of WO ${cur.obj.type}`);
                            if (isEmpty(cur.obj)) {
                                return acc;
                            } else {
                    ...
                    insertObjects = rmArrObjDups(insertObjects,
                        'timeSubmitted');
                    log.error(`MANAGER (198) objects:->${JSON.stringify(
                        insertObjects)}`);
            ...
            }).then((docs) => this.emailWO(docs)).then((docs) => {
                const promises = [];
                newArr.forEach((item) => {
                    // for duplicate items
                    if (item.found) {
    ...
    workOrderSchema.statics.updateDoc = function (id, data, identity) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (data.netsuiteSyned && !data.timeSynced && !data.syncedBy) {
                ...
                this.findById(id).exec().then((doc) => {
                ...
                log.error(
                    `MANAGER (297) !netsuiteSyned || synced -> fn:updateDoc`);
    ...
    workOrderSchema.statics.diff = function (id, newData, identity) {
        ...
        return new Promise((resolve, reject) => {
            if (!identity) return reject(new AuthError("Not Authenticated"));
            if (typeof id === "string") id = ObjectId(id);
            this.findById(id).exec().then((oldData) => {
                if (oldData) {
    ...
    workOrderSchema.statics.UpdateToSync = function (id, doc, identity) {
        return new Promise((resolve, reject) => {
            ...
            log.error(`MANAGER (413) fn:UpdateToSync: doc: ${JSON.stringify(
                approvedDoc)}`);
    ...
    workOrderSchema.statics._count = function (options) {
        return new Promise((resolve, reject) => {
            ...
            if (userQuery) {
                ...
                this.find(query).count().then(resolve).catch(reject);
            }
        });
    };
    ...
    workOrderSchema.statics.woPayrollDump = function (options) {
        return new Promise((resolve, reject) => {
            ...
            if (userQuery) {
                ...
                payrollDump(this, query).then(resolve).catch(reject);
            }
        });
    };
    ...
    workOrderSchema.statics.woDumpReport = function (options) {
        return new Promise((resolve, reject) => {
            ...
            if (userQuery) {
                ...
                woDump(this, query).then(resolve).catch(reject);
            }
        });
    };
    ...
    workOrderSchema.statics.woPartsDumpReport = function (options) {
        return new Promise((resolve, reject) => {
            ...
            if (userQuery) {
                ...
                woPartsDump(this, User, query).then(resolve).catch(reject);
            }
        });
    };
    ...
    workOrderSchema.statics.delete = function (id) {
        return new Promise((resolve, reject) => {
            ...
            this.findOneAndRemove({_id: id}).exec().then(resolve).catch(reject);
        });
    };
    ...
    workOrderSchema.statics.getUnitWorkOrders = function (options) {
        return new Promise((resolve, reject) => {
            ...
            this.list(options).then(resolve).catch(reject);
        });
    };
    ...
    const populateTechByTechId = (wo) => {
        return new Promise((resolve, reject) => {
            if (Object.prototype.toString.call(wo) === '[object Null]') {
                ...
                wo = wo.toObject();
                User.findOne({username: wo.techId}).lean().exec().then(tech => {
