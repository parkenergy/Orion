  ...
  workOrderSchema.statics.timeReport = function (query) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      self.aggregate()
        ...
        .group({
          _id: "$techId",
          timeStarted: {$push: '$timeStarted'},
          timeSubmitted: {$push: '$timeSubmitted'},
          timeLogged: {$sum: '$timeLogged'},
        ...
        .map(function (report) {
          ...
          var timeStarted = new Date(report.timeStarted.toString());
          var timeSubmitted = new Date(report.timeSubmitted.toString());

          // Get difference in minutes
          var loggedMinutes = (timeSubmitted.getTime() - timeStarted.getTime()) * 1000 * 60;

          return {
            tech: report._id,
            lcTime: minToTimeStr(lcMinutes),
            timeLogged: minToTimeStr(loggedMinutes)
          }
        })
        // Reduce results to CSV format
        .reduce(function (csv, report) {
          return csv + [report.tech, report.lcTime, report.timeLogged].join(',') + '\n';
        }, '')
        // Prepend header and finalize results
        .then(function (csv) {
          return 'TECH,TIME LOGGED,LC TIME\n' + csv;
        })
        .then(resolve)
        .catch(reject);
    });
...
function laborCodesToMinutes(lc) {
  ...
  _.forIn(lc, function (v) {
    ...
    _.forIn(v, function (v, k) {
      ...
      if((k !== 'negativeAdj') && (k !== 'positiveAdj') && (typeof k === 'number')) {
        totalMin += v.hours * 60;
        totalMin += v.minutes;
      }
    });
