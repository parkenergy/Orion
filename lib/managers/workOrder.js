  ...
  workOrderSchema.statics.createDoc = function (data) {
    return new Promise((resolve, reject) => {
      ...
      Promise.join(
          ...
          _id: {
            ...
            $in: dataArr.map((obj) => {
              ...
              if (obj.hasOwnProperty("unit")) {
                console.log(1.1);
                if (typeof obj.unit === 'string' && obj.unit.length === 24) {
                  console.log(2.1);
                  obj.unit = ObjectId(obj.unit);
                  return obj.unit;
                } else if (typeof obj.unit === 'object') {
                  console.log(3.1);
                  if (obj.unit.hasOwnProperty("id") && obj.unit.id.length === 24) {
                    console.log(4.1)
                    obj.unit.id = ObjectId(obj.unit.id);
                    return obj.unit.id;
                  } else if (obj.unit.toString().length === 24) {
                    console.log(7.1)
                    return obj.unit;
                  }
                }
        ...
        State.find({
          name: {
            $in: _.map(dataArr, 'header.state')
          }
        }),
        County.find({
          name: {
            $in: _.map(dataArr, 'header.county')
          }
        }),
        (techDocs, unitDocs, stateDocs, countyDocs) => {
          dataArr = dataArr.map((doc) => {
            ...
            const unit = _.find(unitDocs, (o) => {
              ...
              if (doc.hasOwnProperty('unit')) {
                console.log(1);
                // if doc.unit is the id and is a string but not an ObjectId
                /*if (typeof doc.unit === 'string' && doc.unit.length === 24) {
                  console.log(2);
                  
                  // if the unit._id id is a string
                  if (typeof o._id === 'string' && o._id.length === 24) {
                    ...
                    console.log(4);
                    return o._id.toString() === doc.unit;
                  }
                  
                  // else if the doc.unit is an object
                } else */
                if (typeof doc.unit === 'object') {
                  console.log(5);
                  
                  // if the doc.unit has the id and is not itself an ObjectcId
                  if (doc.unit.hasOwnProperty('id')) {
                    // if the doc.uni.id is the id ans is a string
                    if (typeof doc.unit.id === 'string' && doc.unit.id.length === 12) {
                      console.log(7);
                      return o._id.toString() === ObjectId(doc.unit.id).toString();
                      
                      // else the doc.unit.id is an ObjectId
                    } else if (typeof doc.unit.id === 'object') {
                      console.log(10);
                      // if the unit._id is a string
                      return o._id.toString() === doc.unit.id.toString();
                      /*if (typeof o._id === 'string' && o._id.length === 24) {
                        console.log(11);
                        return o._id === doc.unit.id.toString();
                    ...
                    // the doc.unit is in fact an ObjectId
                  }
                  /*else {
																		 console.log(13);
																		 if (typeof o._id === 'string' && o._id.length === 24) {
																			 console.log(14);
																			 return o._id === doc.unit.toString();
																		 } else if (typeof o._id === 'object') {
																			 console.log(15);
																			 return o._id.toString() === doc.unit.toString();
																		 }
																	 }*/
            ...
            if (unit) {
              // populate state
              const state = _.find(stateDocs, (o) => {
                return doc.header.state === o.name;
              });
              log.info({stateFound: state}, "actual state found");
              // populate county
              const county = _.find(countyDocs, (o) => {
                return doc.header.county === o.name;
              });
              log.info({countyFound: county}, "actual county found");
              unit.state = state.name;
              unit.county = county.name;
              log.info({unit: unit}, "actual unit checked");
              ...
              if (doc.type !== "Indirect" && !billable) {
                console.log(getWorkOrderUnitInfo(doc))
                console.log(compareWorkorderUnitToUnit(getWorkOrderUnitInfo(doc), unit))
                if (compareWorkorderUnitToUnit(getWorkOrderUnitInfo(doc), unit)) {
                  // the doc matches 100% with the current unit state
                  syncToNetsuite(doc)
                  .then((res) => {
                    log.trace("Update Inserted doc");
                    const now = new Date();
                    res.updated_at = now;
                    res.timeSynced = now;
                    res.timeApproved = now;
                    res.managerApproved = true;
                    res.approvedBy = res.techId;
                    res.syncedBy = res.techId;
                    // return doc to be inserted after changes.
                    return res;
                  })
                  .catch((err) => {
                    // still return doc to be inserted if error, It will show up
                    // like normal on the manager's dash
                    log.debug({err}, "Sync Error during work order insertion.");
                    return doc
                  });
                }
              }
...
const compareWorkorderUnitToUnit = (wo, unit) => {
  console.log(wo.number === unit.number);
  console.log(wo.geo.coordinates[0] === unit.geo.coordinates[0])
  console.log(wo.geo.coordinates[1] === unit.geo.coordinates[1])
  console.log(wo.state === unit.state)
  console.log(wo.county === unit.county)
  console.log(wo.locationName === unit.locationName)
  console.log(wo.customerName === unit.customerName)
  console.log(wo.engineSerial === unit.engineSerial)
  console.log(wo.compressorSerial === unit.compressorSerial)
  return (wo.number === unit.number &&
    wo.geo.coordinates[0] === unit.geo.coordinates[0] &&
    wo.geo.coordinates[1] === unit.geo.coordinates[1] &&
    wo.state === unit.state &&
    wo.county === unit.county &&
