...
module.exports = function (workOrderSchema) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if(data.netsuiteSyned) {
        ...
        self.findById(id)
          .exec()
          .then(function (doc) {
            if(doc.netsuiteSync) throw Error("Workorder Already Synced");
  ...
  workOrderSchema.statics.list = function (options) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if(options.tech) {
        var nameArr = options.tech.split(' ');

        if(nameArr.length < 2) return reject(new Error("Name is invalid, required first and lastname"));

        userQuery = {
          firstName: {$regex: nameArr[0], $options: 'i'},
          lastName: {$regex: nameArr[1], $options: 'i'}
        };
      ...
      if(options.hideSynced) {
        query.$or = [
          {netsuiteSyned: false},
          {netsuiteSyned: {$exists: false}}
        ];
      }
      ...
      if(userQuery){
        User.find(userQuery, {username: 1})
          ...
          .then(function (workorders) {
            log.trace({wo: workorders}, "Workorders from tech");
            workorders.map(function (wo) {
              if(wo.technician) {
                wo.technician.fullName = [wo.technician.firstName, wo.technician.lastName].join(' ');
                log.trace({name: wo.technician.fullName}, "Fullname");
              }
            });
            return workorders;
          })
...
function populateTechByTechId(techId) {
...
function populateCustomerByName(wo) {
  ...
  return new Promise(function (resolve, reject) {
    if(!wo.header.customerName) return reject(new Error("Missing customerName on Workorder to populate"));
...
function syncToNetsuite(doc) {
  ...
  return new Promise(function (resolve, reject) {
    if(!doc.header.unitNumberNSID) {
      doc.header.unitNumberNSID = doc.unitNumber;
    }

    if(!doc.header.unitNumberNSID) {
      log.debug({doc: doc}, "Indirect or missing unit NSID");
      return reject(new Error("Indirect WorkOrders will not be synced"));
    }
