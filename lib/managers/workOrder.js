...
const queryConstructor = (options, userQuery, query) => {
  ...
  // if any billable is selected. ( item || item ) && ( item || item)
  if((options.billable || options.billParts) && (options.approved || options.unapproved)) {
    query.$and = [ // for exclusive items
      {$or: []}, // for billable items inclusive
      {$or: []}  // for approvals inclusive
    ];
    // inclusive
    if(options.billable) {
    ...
    // inclusive
    if(options.approved){
      query.$and[1].$or.push({ managerApproved: true });
    }
    // inclusive
    if(options.unapproved){
      query.$and[1].$or.push({ managerApproved: false }, { managerApproved: { $exists: false }});
    }
    
  } else if(options.approved || options.unapproved) {
    query.$and = [
      { $or: [] }
    ];
    if(options.approved){
    ...
    if(options.unapproved){
      query.$and[0].$or.push({ managerApproved: false }, { managerApproved: { $exists: false }});
    }
    
  } else if(options.billParts || options.billable) {
    query.$and = [
      { $or: [] }
    ];
    if(options.billable){
  ...
  if(options.billed){
    if(query.$and){
      query.$and.push({ "billingInfo.billed": true });
    } else {
      query.$and = [{ "billingInfo.billed": true }]
    }
  }
  
  if(options.hasOwnProperty('synced')){
    if(query.$and){
      query.$and.push({ netsuiteSyned: options.synced });
    } else {
      query.$and = [{ netsuiteSyned: options.synced }];
    }
  }
