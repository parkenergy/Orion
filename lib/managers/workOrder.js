...
module.exports = function (workOrderSchema) {
                ...
                .then((objects) => {
                    ...
                    if (!isEmpty(objects)) {
                        insertObjects = objects.reduce((acc, cur) => {
                            if (cur.found === false) {
                                log.error(`Type of WO ${cur.obj.type}`)
                                if (isEmpty(cur.obj)) {
                                    return acc
                                } else {
                        ...
                        log.error({objlength: insertObjects}, 'objects being inserted')
    ...
    workOrderSchema.statics.updateDoc = function (id, data, identity) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (data.netsuiteSyned && !data.timeSynced && !data.syncedBy) {
                ...
                log.error('Update doc not synced not syned')
                this.findById(id)
                    .exec()
                    .then((doc) => {
                ...
                log.error('else is synced ?and syned')
    ...
    workOrderSchema.statics.simpleUpdateAndApprove = function (id, doc, identity) {
        return new Promise((resolve, reject) => {
            if (!identity) return reject(new AuthError("Not authenticated"));
            if (typeof id === 'string') id = ObjectId(id);
            log.error('simple update and aprove MANAGER')
    ...
    workOrderSchema.statics.UpdateToSync = function (id, doc, identity) {
        return new Promise((resolve, reject) => {
            if (!identity) return reject(new AuthError("Not authenticated"));
            if (typeof id === 'string') id = ObjectId(i)
            let approvedDoc = doc;
 ...
 * @param wo
 * @returns {{engineSerial: (unitReadings.engineSerial|{type}|string), compressorSerial:
 *     (unitReadings.compressorSerial|{type}|string), number: *, state: *, county: (string|county|{type, ref,
 *     autopopulate}|header.county|{type}|unitSnapShot.county|*), customerName: string, locationName:
