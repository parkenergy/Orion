...
module.exports = function (workOrderSchema) {
  ...
  workOrderSchema.statics.createDoc = function (data) {
    return new Promise((resolve, reject) => {
          ...
          if (unit) {
            ...
            if (doc.type !== "Indirect" && !billable) {
              if (compareWorkorderUnitToUnit(getWorkOrderUnitInfo(doc), unit)) {
                ...
                return {obj: doc, sync: true};
                // promises.push(this.AutoSyncToNetsuite(doc));
              } else {
                return {obj: doc, sync: false};
      ...
      .then((objects) => {
        console.log('get')
        let insertObjects = objects.map((o) => o.obj);
        return this.insertMany(insertObjects);
      })
      .then((docs) => { console.log('send'); return emailWO(docs);})
      .then((docs) => {
        console.log('sync');
        const promises = [];
        newArr.forEach((item) => {
          docs.forEach((document) => {
            console.log(typeof document.timeSubmitted);
            console.log(typeof item.obj.timeSubmitted);
            if (document.timeSubmitted.toString() === new Date(item.obj.timeSubmitted).toString()) {
              if (item.sync) {
...
const compareWorkorderUnitToUnit = (wo, unit) => {
  let state;
  let county;
  console.log(unit.county);
  console.log(unit.state);
  if (unit.state !== null) {
    if (unit.state.hasOwnProperty('name')) {
  ...
  if (unit.county !== null) {
    if (unit.county.hasOwnProperty('name')) {
