...
module.exports = function (workOrderSchema) {
                        ...
                        $in: dataArr.map((obj) => {
                            if (obj.hasOwnProperty("unit") && obj !== null && obj !== undefined) {
                                if (obj.unit !== null && obj.unit !== undefined && obj.unit !== '') {
                                    return typeof obj.unit === 'string' ? ObjectId(obj.unit) : obj.unit;
                                } else {
                                    return null;
                                }
                            } else {
            ...
            //Insert docs
                        ...
                        if (unit) {
                            ...
                            if ((doc.type !== "Indirect" && doc.type !== 'Swap' && doc.type !== 'Transfer') && !billable ) {
                                if (compareWorkorderUnitToUnit(getWorkOrderUnitInfo(doc), unit) && LaborCodePasses(doc)) {
                                    ...
                                    return {obj: doc, sync: true, found: false, managerSync: false};
                                } else {
                                    return {obj: doc, sync: false, found: false, managerSync: false}
                                }
                            } else {
                                return {obj: doc, sync: false, found: false, managerSync: false};
                            }
                ...
                .then((docs) => {
                    ...
                    newArr.forEach((item) => {
                        // for duplicate items
                        if (item.found) {
                            // return the found object because it has already been synced.
                            promises.push(new Promise((res) => res(item.obj)));
                        } else {
                            // for items that were new
                            docs.forEach((document) => {
                                if ((document.timeSubmitted.toString() === new Date(item.obj.timeSubmitted).toString()) && (item.found === false)) {
                                    if (item.sync) {
                                        const SyncDoc = item.obj;
                                        SyncDoc._id = document._id ? document._id : '';
                                        SyncDoc.id = document.id ? document.id : '';
                                        // prevent from submitting items.length * docs.length
                                        item.found = true;
                                        // for testing
                                        ...
                                        promises.push(this.find({techId: updateDoc.techId, timeSubmitted: updateDoc.timeSubmitted, timeCreated: updateDoc.timeCreated})
                                            .exec()
                                            .then((docs) => {
                                                if (docs.length > 0) {
                                                    const doc = docs[0];
                                                    return this.findByIdAndUpdate(doc._id, updateDoc, {safe: false, new: true})
                                                        .exec()
                                                } else {
                ...
                .then(resolve)
                .catch((err) => {
                    log.error({error: err}, "Error Occured during Orion Insert: " + err);
                    reject(err);
                })
        });
    };
        ...
        return new Promise((resolve, reject) => {
            ...
            if (data.netsuiteSyned && !data.timeSynced && !data.syncedBy) {
                ...
                this.findById(id)
                    .exec()
                    .then((doc) => {
                        if (doc.timeSynced && doc.syncedBy && doc.netsuiteSyned) throw new ClientError("Workorder Already Synced");
        ...
        return new Promise((resolve, reject) => {
            ...
            this.diff(id, doc, identity)
                .then((data) => {
                    ...
                    return this.findByIdAndUpdate(id, ApprovedData, {save: false, new: true})
                        .lean()
                        .exec();
                })
                .then(resolve)
    ...
    /**
     * For when teh manager approves a work order that has no
    ...
    workOrderSchema.statics.UpdateToSync = function (id, doc, identity) {
        return new Promise((resolve, reject) => {
            ...
            if (typeof id === 'string') id = ObjectId(id);
            let approvedDoc;
            if (!doc.managerApproved && !doc.timeApproved && !doc.managerApproved) {
    ...
    workOrderSchema.statics.unapprovedByArea = function (users) {
        return new Promise((resolve, reject) => {
            ...
            Promise.all(TheseUsers.map((tech) => {
                return this.privateAggregateArea(tech, {area: tech.area, count: 0});
            }))
                .then((all) => {
                    resolve(all);
                })
            ...
            if (userQuery) {
                User.find(userQuery, {username: 1})
                    ...
                    .then((users) => {
                        ...
                        query.techId = {$in: userNames};
                        if (options.report) {
                            return payrollDump(this, query);
                        }
                    })
                    .then(resolve)
                    .catch(reject);
            } else {
                payrollDump(this, query)
                    .then(resolve)
                    .catch(reject);
            }
        });
    ...
    workOrderSchema.methods.returnApprovedDoc = function(doc, identity) {
                ...
                Promise.all(wos.map((wo) => {
                    ...
                    .then((workorders) => {
                        ...
                        workorders.forEach((wo) => {
                            if ((new Date(wo.timeSubmitted).getTime() < times.lastDay.getTime()) && (new Date(wo.timePosted).getTime() > times.checkDay.getTime())) {
                                let mailOptions = {
                                    from: '"Orion Alerts" <orionalerts@parkenergyservices.com>',
                                    to: to,
                                    subject: `LATE SUBMISSION | ${wo.type} | ${wo.header.unitNumber} | ${wo.techId} | ${new Date(wo.timeSubmitted).toLocaleString()}`,
                                    // text: ,
                                    html: `
                  <body>
                  ...
                  `,
                    ...
                    .catch((err) => {
                        console.log(err);
                        log.error({error: err}, "Issue emailing out late wos" + err);
                    })
            }
        });
    };
                    ...
                    mailer.transport.verify(function (error, success) {
                        if (error) {
                            ...
                            let mailOptions = {
                                from: '"Orion Alerts" <orionalerts@parkenergyservices.com>',
                                to: to,
                                subject: `${wo.type} | ${wo.header.unitNumber} | ${wo.header.customerName} | ${wo.header.leaseName} | ${replace}`,
                                // text: ,
                                html: `
              <body>
                  <a href="http://orion.parkenergyservices.com/#/workorder/review/${wo._id}">Link to Work Order</a>
                    ...
                    mailer.transport.verify(function (error, success) {
                        if (error) {
                            ...
                            let mailOptions = {
                                from: '"Orion Alerts" <orionalerts@parkenergyservices.com>',
                                to: to,
                                subject: `${wo.type} | ${wo.header.unitNumber} | ${wo.header.customerName} | ${wo.header.leaseName} | Replacement!`,
                                // text: ,
                                html: `
              <body>
                  <a href="http://orion.parkenergyservices.com/#/workorder/review/${wo._id}">Link to Work Order</a>
                ...
                mailer.transport.verify(function (error, success) {
                    if (error) {
                        ...
                        let mailOptions = {
                            from: '"Orion Alerts" <orionalerts@parkenergyservices.com>',
                            to: to,
                            subject: `${wos.type} | ${wos.header.unitNumber} | ${wos.header.customerName} | ${wos.header.leaseName} | ${replace}`,
                            // text: ,
                            html: `
            <body>
                ...
                mailer.transport.verify(function (error, success) {
                    if (error) {
                        ...
                        let mailOptions = {
                            from: '"Orion Alerts" <orionalerts@parkenergyservices.com>',
                            to: to,
                            subject: `${wos.type} | ${wos.header.unitNumber} | ${wos.header.customerName} | ${wos.header.leaseName} | Replacement!`,
                            // text: ,
                            html: `
            <body>
                <a href="http://orion.parkenergyservices.com/#/workorder/review/${wos._id}">Link to Work Order</a>
...
/**
 * Used to get the wo unit information of just the serial numbers and coordinates
 * @param wo
 * @returns {{engineSerial: (unitReadings.engineSerial|{type}|string), compressorSerial: (unitReadings.compressorSerial|{type}|string), geo: *}}
 */
/*const getWorkOrderUnitSerialCoors = (wo) => ({
...
/**
 * Used to compare both the netsuite unit info to the work order
 * unit info. With a tolerance on the coordinates.
 * @param wo
 * @param unit
 * @returns {boolean}
 */
/*const compareWorkorderUnitToUnitSerialCoords = (wo, unit) => {
    // don't block of lat long anymore
    /!*console.log(percentDiff(wo.geo.coordinates[0], unit.geo.coordinates[0]))
    console.log(percentDiff(wo.geo.coordinates[1], unit.geo.coordinates[1]))
    return ((percentDiff(wo.geo.coordinates[0], unit.geo.coordinates[0]) < 0.006) &&
      (percentDiff(wo.geo.coordinates[1], unit.geo.coordinates[1]) < 0.006) &&
      (wo.engineSerial === unit.engineSerial) &&
      (wo.compressorSerial === unit.compressorSerial));*!/
    return (wo.engineSerial === unit.engineSerial) &&
        (wo.compressorSerial === unit.compressorSerial);
};*/
    ...
    return new Promise((resolve, reject) => {
        ...
        populateCustomerByName(doc)
            .then((wo) => {
                ...
                return axios.post('https://rest.na1.netsuite.com/app/site/hosting/restlet.nl?script=112&deploy=1', nswo, {
                    headers: {
