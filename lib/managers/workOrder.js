      ...
      Promise.join(
        ...
        Unit.find({
          _id: {
            ...
            $in: dataArr.map((obj) => {
              log.trace({issueObj: obj}, "object in question");
        ...
        (techDocs, unitDocs, stateDocs, countyDocs) => {
          dataArr = dataArr.map( (doc) =>  {
            ...
            log.trace({units: unitDocs}, "units found");
            // Pull the unit from the array of found Units with that netsuite ID
            const unit = _.find(unitDocs, (o) => {
              if (Object.prototype.toString.call(doc.unit) === '[object String]') {
                return doc.unit === o._id.toString();
              } else if (Object.prototype.toString.call(doc.unit) === '[object Object]') {
                return doc.unit.toString() === o._id.toString();
              } else {
                return doc.unit === o._id;
              }
            });
            log.trace({unitFound: unit}, "Actual unit found");
            ...
            if(unit) {
              ...
              log.trace({stateFound: state}, "actual state found");
              // populate county
              const county = _.find(countyDocs, (o) => {
                return doc.header.county === o.name;
              });
              log.trace({countyFound: county}, "actual county found");
              unit.state = state;
              unit.county = county;
