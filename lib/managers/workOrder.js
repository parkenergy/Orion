...
module.exports = function (workOrderSchema) {
        ...
        return new Promise((resolve, reject) => {
            ...
            Promise.all([
                ...
                .then((docs) => {
                    ...
                    const swapUnits = docs[2]
                    const FoundWOs = docs[3]
                    newArr = dataArr.reduce((acc, doc) => {
                        ...
                        if (doc.hasOwnProperty('unit') && doc.unit !== null) {
                            unit = _.find(unitDocs, (o) => {
                                return `${o._id}` === `${doc.unit}`;
                            });
                        }
                        let swapUnit = null
                        if (!isEmpty(doc.unitChangeInfo.swapUnitNumber)) {
                            swapUnit = _.find(swapUnits, (o) => {
                                return o.number === doc.unitChangeInfo.swapUnitNumber
                            })
                        }
                        ...
                        if (unit) {
                            ...
                            if ((doc.type !== 'Indirect' && doc.type !== 'Swap' && doc.type !==
                                ...
                                if (compareWorkorderUnitToUnit(getWorkOrderUnitInfo(doc), !isEmpty(swapUnit)
                                    ? swapUnit
                                    : unit) && LaborCodePasses(doc)) {
                                    /**
                                     ...
                                     */
                                    if (compareWorkorderUnitToUnitSerial(getWorkOrderUnitSerial(doc), !isEmpty(swapUnit)
                                        ? swapUnit
                                        : unit) && LaborCodePasses(doc)) {
                ...
                .then((objects) => {
                    ...
                    if (!isEmpty(objects)) {
                        insertObjects = objects.reduce((acc, cur) => {
                            if (cur.found === false) {
                                // log.error(`MANAGER (190) Type of WO ${cur.obj.type}`);
                                if (isEmpty(cur.obj)) {
                                    return acc;
                                } else {
                        ...
                        if (isEmpty(insertObjects)) {
                            ...
                            //log.error(`MANAGER (205) objects:->${insertObjects.length}`);
    ...
    workOrderSchema.statics.updateDoc = function (id, data, identity) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (data.netsuiteSyned && !data.timeSynced && !data.syncedBy) {
                ...
                // log.error(`MANAGER (274) netsuiteSyned && !synced -> fn:updateDoc`);
    ...
    workOrderSchema.statics.UpdateToSync = function (id, doc, identity) {
        return new Promise((resolve, reject) => {
            ...
            // log.error(`MANAGER (419) fn:UpdateToSync: doc: ${JSON.stringify(approvedDoc)}`);
...
const syncToNetsuite = (doc) => {
    ...
    return new Promise((resolve, reject) => {
        if (!doc.header.unitNumberNSID && doc.type !== 'Swap') {
            doc.header.unitNumberNSID = doc.unitSnapShot.netsuiteId
        }
        if (!doc.unitChangeInfo.swapUnitNSID && doc.type === 'Swap') {
            doc.unitChangeInfo.swapUnitNSID = doc.unitSnapShot.netsuiteId
        }
        if (!doc.header.unitNumberNSID && doc.type === 'Swap') {
            doc.header.unitNumberNSID = doc.unitSnapShot.netsuiteId
        }

        if (doc.type === 'Indirect') {
        ...
        async function getCustomer () {
            const wo = await populateCustomerByName(doc)
            nswo = netSuiteFormat(wo)
            let backoff = 200
            let retries = 8
            ...
            const makeCall = () => {
                ...
                //create a link
                var myWorkOrders = nsrestlet.createLink(accountSettings, urlSettings)
                //then call get, post, put, or delete
                var workOrder_link = nsrestlet.createLink(accountSettings, urlSettings);
                //... and so do promises.
                workOrder_link.post(nswo)
                .then(function(body)
                {
                    console.log(body);
                })
                ...
                //workOrder_link also has .put() and .delete() methods
            }
            ...
            //     let NETSUITE_ACCOUNT_ID = '4086435'
            //     let BASE_URL = 'https://rest.na1.netsuite.com/app/site/hosting/restlet.nl'
            //     let SCRIPT_ID = '112'
            //     let SCRIPT_DEPLOYMENT_ID = '1'
            //     let HTTP_METHOD = 'POST'
            //     let OAUTH_VERSION = '1.0'
            //     let TOKEN_ID = '91a98fb2380a49f47700e2f88112935af7cc5081aa8f72d8bf5e5f2df49d72cb'
            //     let TOKEN_SECRET = 'e6d9706fef2a5f270f947b49ccaaca213aae5513d6077681db57ffa23a2e6307'
            //     let CONSUMER_KEY = '747d3d1d175c4e866938e8712b90e420606981c5e389acb140c74eec5cfb6674'
            //     let CONSUMER_SECRET = '38b57c0910b0b920f2f8ba1c2d0d4781418937be6cf2bfc9ee5b3c8532b58a37'
            //     let text = ''
            ...
            //     }
            //     let OAUTH_NONCE = text

            //     let TIME_STAMP = Math.round(+new Date() / 1000)
            ...
            //     let completeData = HTTP_METHOD + '&' + encodeURIComponent(BASE_URL) + '&' +
            //         encodedData
            //     let hmacsha1Data = CryptoJS.HmacSHA256(completeData, CONSUMER_SECRET + '&' +
            //         TOKEN_SECRET)
            //     let base64EncodedData = Base64.stringify(hmacsha1Data)
            ...
            //     let url = BASE_URL + '?script=' + SCRIPT_ID + '&deploy=' + SCRIPT_DEPLOYMENT_ID
            //     let header = {'Authorization': OAuth, 'Content-Type': 'application/json'}
            //     nswo = JSON.stringify(nswo, replacer)
            ...
            //         }
            //         if (resp.body.error) {
            //             const error_message = resp.body.error.code ||
            //                 JSON.stringify(resp)
    ...
    try {
        return {
            ...
            compOilPressKill:      wo.unitReadings.compOilPressKill
                                       ...
                                       : '',
            compOilTemp:           wo.unitReadings.compOilTemp ? wo.unitReadings.compOilTemp : '',
            ...
            afrModel:                wo.emissionsReadings.afrModel
                                         ...
                                         : '',
            afrSN:                   wo.emissionsReadings.afrSN ? wo.emissionsReadings.afrSN : '',
            ...
            engineVibrationSwitch: formatLaborCodeTime(wo.laborCodes.engine.engineVibrationSwitch),
            engineBelts:           formatLaborCodeTime(wo.laborCodes.engine.engineBelts),
            harnessRepair:         formatLaborCodeTime(wo.laborCodes.engine.harnessRepair),
            EICSSensorActuators:   formatLaborCodeTime(wo.laborCodes.engine.EICSSensorActuators),
            ...
            o2SensorReplace:         formatLaborCodeTime(wo.laborCodes.emissions.o2SensorReplace),
            catalystReplace:         formatLaborCodeTime(wo.laborCodes.emissions.catalystReplace),
            emissionsThermocoupling: formatLaborCodeTime(wo.laborCodes.emissions.emissionsThermocoupling),
            exhaustGasketReplace:    formatLaborCodeTime(wo.laborCodes.emissions.exhaustGasketReplace),
            facilitySetup:           formatLaborCodeTime(wo.laborCodes.emissions.facilitySetup),
            adjustment:              formatLaborCodeTime(wo.laborCodes.emissions.adjustment),
            troubleshooting:         formatLaborCodeTime(wo.laborCodes.emissions.troubleshooting),
            standBy:                 formatLaborCodeTime(wo.laborCodes.emissions.standBy),

            //panel labor codes
            panel:           formatLaborCodeTime(wo.laborCodes.panel.panel),
            electrical:      formatLaborCodeTime(wo.laborCodes.panel.electrical),
            wiring:          formatLaborCodeTime(wo.laborCodes.panel.wiring),
            conduit:         formatLaborCodeTime(wo.laborCodes.panel.conduit),
            gauges:          formatLaborCodeTime(wo.laborCodes.panel.gauges),
            panelDampners:   formatLaborCodeTime(wo.laborCodes.panel.panelDampners),
            tubing:          formatLaborCodeTime(wo.laborCodes.panel.tubing),
            programming:     formatLaborCodeTime(wo.laborCodes.panel.programming),
            annuciator:      formatLaborCodeTime(wo.laborCodes.panel.annuciator),
            safetyShutdowns: formatLaborCodeTime(wo.laborCodes.panel.safetyShutdowns),
            ...
            compressorAddOil:          formatLaborCodeTime(wo.laborCodes.compressor.addOil),
            lubePump:                  formatLaborCodeTime(wo.laborCodes.compressor.lubePump),
            valves:                    formatLaborCodeTime(wo.laborCodes.compressor.valves),
            alignment:                 formatLaborCodeTime(wo.laborCodes.compressor.alignment),
            piston:                    formatLaborCodeTime(wo.laborCodes.compressor.piston),
            packing:                   formatLaborCodeTime(wo.laborCodes.compressor.packing),
            compressorThermocouples:   formatLaborCodeTime(wo.laborCodes.compressor.compressorThermocouples),
            noFlowSwitch:              formatLaborCodeTime(wo.laborCodes.compressor.noFlowSwitch),
            overhaul:                  formatLaborCodeTime(wo.laborCodes.compressor.overhaul),
            compressorVibrationSwitch: formatLaborCodeTime(wo.laborCodes.compressor.compressorVibrationSwitch),

            //cooler labor codes
            cooling:               formatLaborCodeTime(wo.laborCodes.cooler.cooling),
            coolTubeRepair:        formatLaborCodeTime(wo.laborCodes.cooler.coolTubeRepair),
            leakTesting:           formatLaborCodeTime(wo.laborCodes.cooler.leakTesting),
            pluggingCoolerTube:    formatLaborCodeTime(wo.laborCodes.cooler.pluggingCoolerTube),
            flushCooler:           formatLaborCodeTime(wo.laborCodes.cooler.flushCooler),
            washCooler:            formatLaborCodeTime(wo.laborCodes.cooler.washCooler),
            coolerBelts:           formatLaborCodeTime(wo.laborCodes.cooler.coolerBelts),
            shaftBearing:          formatLaborCodeTime(wo.laborCodes.cooler.shaftBearing),
            idlerBearing:          formatLaborCodeTime(wo.laborCodes.cooler.idlerBearing),
            fan:                   formatLaborCodeTime(wo.laborCodes.cooler.fan),
            shivePulley:           formatLaborCodeTime(wo.laborCodes.cooler.shivePulley),
            coolerVibrationSwitch: formatLaborCodeTime(wo.laborCodes.cooler.coolerVibrationSwitch),
            ...
            dumpValve:        formatLaborCodeTime(wo.laborCodes.vessel.dumpValve),
            piping:           formatLaborCodeTime(wo.laborCodes.vessel.piping),
            screenWitchesHat: formatLaborCodeTime(wo.laborCodes.vessel.screenWitchesHat),
            vesselDampners:   formatLaborCodeTime(wo.laborCodes.vessel.vesselDampners),
