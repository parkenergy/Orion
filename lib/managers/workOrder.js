          ...
          if (unit) {
            ...
            if ((doc.type !== "Indirect" && doc.type !== 'Swap' && doc.type !== 'Transfer') && !billable ) {
              if (compareWorkorderUnitToUnit(getWorkOrderUnitInfo(doc), unit) && LaborCodePasses(doc)) {
                ...
                if(compareWorkorderUnitToUnitSerialCoords(getWorkOrderUnitSerialCoors(doc), unit) && LaborCodePasses(doc)) {
                  return {obj: doc, sync: false, found: false, managerSync: true};
                } else {
    ...
    mailer.transport.verify((error, success) => {
      if (error) {
        ...
        Promise.all(wos.map((wo) => {
          ...
          .then((workorders) => {
            workorders = tedbUtils.flattenArr(workorders);
            let to;
...
const queryConstructor = (options, userQuery, query) => {
  ...
  * */
  ...
  if (options.hasOwnProperty('supervised') && options.searchSupervisor && options.role === 'admin') {
    if (options.supervised.length > 0) {
