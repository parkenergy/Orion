        ...
        newArr = dataArr.map((doc) => {
          ...
          FoundWOs.forEach((fDoc) => {
            if (fDoc.timeSubmitted.toString() === new Date(doc.timeSubmitted).toString()) {
              foundDup = {obj: fDoc, sync: false, found: true};
            }
          });
          ...
          if (unit) {
            ...
            if (doc.type !== "Indirect" && !billable) {
              if (compareWorkorderUnitToUnit(getWorkOrderUnitInfo(doc), unit) && LaborCodePasses(doc)) {
                ...
                return {obj: doc, sync: true, found: false};
                // promises.push(this.AutoSyncToNetsuite(doc));
              } else {
                return {obj: doc, sync: false, found: false};
...
const LaborCodePasses = (wo) => {
  ...
  // dont let work orders over 8 hours auto sync
  if (totalHours > 8) {
    return false;
  }
  ...
  if (TravelHours !== 0) {
    // prevent auto sync if tech traveled under 25 constantly
    if ((distance/TravelHours) < 25) {
      return false;
    }
    // prevent auto sync if tech was traveling over 75 constantly
    if ((distance/TravelHours) > 75) {
      return false;
    }
  }
  return true;
