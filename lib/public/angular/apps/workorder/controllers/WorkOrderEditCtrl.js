angular.module('WorkOrderApp.Controllers').controller('WorkOrderEditCtrl',
    ['$window', '$q', '$scope', '$location', '$timeout', '$uibModal', '$cookies', 'AlertService', 'TimeDisplayService', 'ApiRequestService', 'GeneralPartSearchService', 'ObjectService', 'CommonWOfunctions', 'WorkOrders', 'ReviewNotes', 'EditHistories', 'Units', 'Users', 'Customers', 'workorder', 'reviewNotes', 'editHistories', 'assettypes', 'me', 'parts', 'states', 'applicationtypes', 'DateService', 'locations',  'woInputMatrixes', 'frameModels', 'engineModels',
        function ($window, $q, $scope, $location, $timeout, $uibModal, $cookies, AlertService, TimeDisplayService, ApiRequestService, GeneralPartSearchService, ObjectService, CommonWOfunctions, WorkOrders, ReviewNotes, EditHistories, Units, Users, Customers, workorder, reviewNotes, editHistories, assettypes, me, parts, states, applicationtypes, DateService, locations, woInputMatrixes, frameModels, engineModels) {
            // set times to server
            const setSave = (wo) => {
                if (wo.timeStarted) {
            ...
            $scope.typeChange = (obj) => {
                ...
                // console.log(type)
                if(obj.text === 'PM' || obj.text === 'PM2' || obj.text === 'PM3' || obj.text === 'PM4' || obj.text === 'PM5') {
                    if(type === 'New Set' || type === 'Release' || type === 'Indirect') {
                    ...
                    } else if ((type === 'Corrective' || type === 'Trouble Call' || type === 'Transfer' || type === 'Swap') && (!obj.value)) {
                        ...
                        // console.log(`3. PM type selected. I am ${obj.text}. my value is ${obj.value}`)
                        
                        if(obj.text !== type) {
                            // console.log(5 + ' the current selected is not the current type')
                            if (type !== 'Corrective' && type !== 'Trouble Call' && type !== 'Transfer' && type !== 'Swap') {
                            ...
                            if(type === 'Corrective' || type === 'Trouble Call' || type === 'Transfer' || type === 'Swap') {
                                ...
                                // console.log(`18.  ${obj.text} was clicked. value is ${obj.value}. Selecting current PM, keeps true.`)
                                if(!obj.value) {
                                    $scope.setPM(obj.text, !obj.value)
                                }
                            }
                    ...
                    // console.log(9)
                    $scope.wipeTypeObj()
                    if(obj.text === 'New Set' || obj.text === 'Release' || obj.text === 'Indirect') {
                        // console.log(10)
                        ...
                        $scope.workorder.pm4 = false
                        $scope.workorder.pm5 = false
                    } else if(obj.text === 'Corrective' || obj.text === 'Trouble Call' || obj.text === 'Transfer' || obj.text === 'Swap') {
                        if($scope.workorder.pm || $scope.workorder.pm2 || $scope.workorder.pm3 || $scope.workorder.pm4 || $scope.workorder.pm5) {
                            // console.log(obj.value)
                            // console.log($scope.type)
                            if((type !== 'PM' && type !== 'PM2' && type !== 'PM3' && type !== 'PM4' && type !== 'PM5') && !obj.value) {
                                // console.log('Not PM and Checked is false')
                                $scope.workorder.type = $scope.workorder.pm ? 'PM' : ($scope.workorder.pm2 ? 'PM2' : ($scope.workorder.pm3 ? 'PM3' : ($scope.workorder.pm4 ? 'PM4' : ($scope.workorder.pm5 ? 'PM5' : obj.text ))))
                                $scope.setTypeObj(obj.text, obj.value)
