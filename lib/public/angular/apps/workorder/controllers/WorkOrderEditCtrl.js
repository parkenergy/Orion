function WorkOrderEditCtrlFunc($window, $q, $scope, $location, $timeout, $uibModal, $cookies, AlertService,
                               TimeDisplayService, ApiRequestService, GeneralPartSearchService, ObjectService,
                               CommonWOfunctions, WorkOrders, ReviewNotes, EditHistories, Units, Users, Customers,
                               workorder, reviewNotes, editHistories, assettypes, me, parts, states, applicationtypes,
                               DateService, locations, woInputMatrixes, frameModels, engineModels, counties) {
               ...
               for (let unit in units) {
                   if (units.hasOwnProperty(unit)) {
                       if ((units[unit].number === changedData) &&
                           ...
                           $timeout(() => {
                               // Fill doc variables
                               $scope.workorder.header.state =
                                   thisUnit.state === null ? '' : $scope.statesObj[thisUnit.state].name
                               $scope.workorder.header.county =
                                   thisUnit.county === null ? '' : $scope.countiesObj[thisUnit.county].name
    ...
    $scope.changeThisTextField = (changedData, selected) => {
        if (selected === 'unitReadings.engineSerial' || selected === 'unitReadings.compressorSerial' || selected ===
            'newEngineSerial' || selected === 'newCompressorSerial') {
            ObjectService.updateNestedObjectValue($scope.workorder,
...
angular
    ...
    .controller('WorkOrderEditCtrl',
        ['$window', '$q', '$scope', '$location', '$timeout', '$uibModal', '$cookies', 'AlertService',
            'TimeDisplayService', 'ApiRequestService', 'GeneralPartSearchService', 'ObjectService', 'CommonWOfunctions',
            'WorkOrders', 'ReviewNotes', 'EditHistories', 'Units', 'Users', 'Customers', 'workorder', 'reviewNotes',
            'editHistories', 'assettypes', 'me', 'parts', 'states', 'applicationtypes', 'DateService', 'locations',
            'woInputMatrixes', 'frameModels', 'engineModels', 'counties', WorkOrderEditCtrlFunc])
