function WorkOrderViewCtrl($scope, $location, $timeout, $uibModal, $cookies, AlertService, ApiRequestService,
    ...
    if ($scope.workorder.unitNumber) {
        setDisplayUnit($scope.workorder.unitNumber, $scope.workorder.unitChangeInfo.swapUnitNumber)
    }
    /**
     *
    ...
    $scope.submit = () => {
        ...
        if ($scope.workorder.emissionsReadings.lastCalibration &&
            ...
            try {
                ...
                allowSubmit = false
                $scope.submittingWorkOrder = false
                AlertService.add('danger', 'Unable to Convert last calibration to a number')
            }
        }

        if ($scope.workorder.header.timeOfCallout &&
            Object.prototype.toString.call($scope.workorder.header.timeOfCallout) !== '[object Number]') {
            try {
                $scope.workorder.header.timeOfCallout = new Date($scope.workorder.header.timeOfCallout).getTime()
            } catch (e) {
                allowSubmit = false
                $scope.submittingWorkOrder = false
                AlertService.add('danger', 'Unable to Convert time of callout back to a number.')
            }
        }
        if ($scope.disabled) {// Review
            ...
            if (!isEmpty($scope.workorder.unitReadings.compressorModel) ||
            ...
            if (!isEmpty($scope.workorder.header.timeOfCallout) && $scope.workorder.header.timeOfCallout < 0) {
                $scope.openErrorModal('woTimeOfCalloutError.html')
                allowSubmit = false
            }
            if ($scope.workorder.emissionsReadings.spotCheck &&
