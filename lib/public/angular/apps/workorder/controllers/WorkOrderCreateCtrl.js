function WorkOrderCreateCtrlFunc ($window, $q, $scope, $location, $timeout, $uibModal, $cookies, AlertService, ApiRequestService, GeneralPartSearchService, ObjectService, CommonWOfunctions, Utils, me) {
    ...
    $scope.submit = () => {
        ...
        if ($scope.workorder.type !== 'Swap' && (isEmpty($scope.workorder.unitNumberNSID) || isEmpty($scope.workorder.unitNumber))) {
        ...
        if (isEmpty($scope.workorder.truckId) || isEmpty($scope.workorder.techId)) {
