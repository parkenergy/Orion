function CommonWOfunctions($timeout, ObjectService, ApiRequestService, TimeDisplayService) {
    ...
    return {
        ...
        getPaidForLaborTime(wo) {
            let totalMin = 0
            const laborCodeKeys = Object.keys(wo.laborCodes)
            laborCodeKeys.forEach((lc) => {
                const subLaborCodeKeys = Object.keys(wo.laborCodes[lc])
                subLaborCodeKeys.forEach((slc) => {
                    const laborCode = wo.laborCodes[lc][slc]
