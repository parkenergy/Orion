class woHeaderInfo {
    constructor($scope, $timeout, ApiRequestService, CommonWOfunctions, Utils, TimeDisplayService) {
        this.utils = Utils
    ...
    $onInit() {
        if ((this.workorder.type === 'Trouble Call' || this.workorder.type === 'Corrective') &&
    ...
    validateState(wo, du, u) {
        ...
        if (unitExists === 'is_unit') {
            let state = (du.state === null || du.state === undefined) ? '' :
                        (!this.utils.isEmpty(this.utils.getObjFromArrayByID(this.states, du.state)) ?
                         this.utils.getObjFromArrayByID(this.states, du.state) :
                         (!this.utils.isEmpty(this.utils.getObjByValue(this.states, du.state, 'name')) ?
    ...
    validateCounty(wo, du, u) {
        ...
        if (unitExists === 'is_unit') {
            let county = (du.county === null || du.county === undefined) ? '' :
                         (!this.utils.isEmpty(this.utils.getObjFromArrayByID(this.counties, du.county)) ?
                          this.utils.getObjFromArrayByID(this.counties, du.county) :
                          (!this.utils.isEmpty(this.utils.getObjByValue(this.counties, du.county, 'name')) ?
    ...
    * */
    ...
    resetInfo(wo, du, u, state, unit, input) {
        if (unit.number.toUpperCase() === input.toUpperCase()) {
            ...
            wo.header.unitNumber = thisUnit.number
            if (thisUnit.county) {
                const county = this.utils.getObjFromArrayByID(this.counties, thisUnit.county)
            ...
            if (thisUnit.state) {
                const state = this.utils.getObjFromArrayByID(this.states, thisUnit.state)
...
angular
    ...
    .component('woHeaderInfo', {
        ...
        controller:  ['$scope', '$timeout', 'ApiRequestService', 'CommonWOfunctions', 'Utils', 'TimeDisplayService',
            woHeaderInfo],
    })
