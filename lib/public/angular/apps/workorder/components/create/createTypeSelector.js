class createTypeSelector {
     ...
     */
    runHeaderValidation () {
        console.log('runHeaderValidation')
        this.toCb((wo, a, b) => {
            console.log('runHeaderValidation CB')
            const header = wo.header
            wo.header = null
            if (wo.type !== 'Swap') {
                this.setDisplayUnit(header.unitNumber)
            ...
            this.timeout(() => {
                console.log('in timeout of runHeaderValidation CB')
                wo.header = header
                console.log(wo.type)
                console.log(wo.pm)
                console.log(wo.pm2)
    ...
    setDisplayUnit (number) {
        this.toCb((wo, du, hu) => {
            if (wo.unitSnapShot &&
                wo.unitSnapShot.hasOwnProperty('number')) {
                this.timeout(() => {
                    du = wo.unitSnapShot
                    hu = wo.unitSnapShot
                })
            } else {
                this.ARS.Units({regexN: number})
                    .then((units) => {
                        this.timeout(() => {
                            du = units[0]
                        })
...
angular
    ...
    .component('createTypeSelector', {
        ...
        controller:  ['$scope', '$timeout', 'ApiRequestService', createTypeSelector],
    })
