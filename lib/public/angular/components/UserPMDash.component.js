class UserPmDashCtrl {
    ...
    // Init values ----------------------------------------
    $onInit () {
        this.fullName = this.user.firstName.concat(' ')
                            .concat(this.user.lastName)

        this.ARS.Users({textId: this.user.supervisor})
            .then((res) => {
                if (res.length > 0) {
                    this.supervisor = res[0].firstName.concat(' ')
...
angular.module('CommonComponents')
       .component('userPmDash', {
           ...
           controller:  ['$window', '$timeout', '$location', 'PmDashService', 'ApiRequestService', 'SessionService', UserPmDashCtrl],
});
