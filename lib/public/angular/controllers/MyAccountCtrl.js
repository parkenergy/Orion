function MyAccountCtrl ($window, $scope, users, units, ApiRequestService,
    SessionService) {
    ...
    ARS.http.get.WorkOrdersUnapprovedArea(obj)
       .then((res) => {
           res.data.forEach((a) => {
               const thisArea = Area()
               thisArea.area = a.area.split(':')[0].slice(2)
                                                   .trim()
               thisArea.count = a.count
               if (searchedAreas.indexOf(thisArea.area) === -1 &&
                   thisArea.area !== '') {
...
angular.module('CommonControllers')
       .controller('MyAccountCtrl', ['$window',
           '$scope',
           'users',
           'units',
           'ApiRequestService',
           'SessionService', MyAccountCtrl]);
