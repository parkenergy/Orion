...
PartOrderCtrl.noAuthCount = (req, res) => {
...
PartOrderCtrl.poDump = (req, res) => {
    ...
    try {
        ...
        PartOrder.poDumpReport(options)
                 .then(result => {
                     res.downloadable('poDump.csv', 'text/csv', result)
                 })
                 .catch(res.reject)
    } catch (e) {
        log.warn('Failed to parse params into date')
