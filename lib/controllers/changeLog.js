...
const ChangeLogCtrl = {
    create (req, res) {
        if (!req.body) return res.reject(
            new ClientError('Missing Body Change Log'));
    ...
    read (req, res) {
        ...
        if (!id) return res.reject(
            new ClientError('Missing Id parameter for ChangeLog'));
    ...
    update (req, res) {
        ...
        if (!body) return res.reject(
            new ClientError('Missing body for changeLoc'));
        if (!id) return res.reject(
            new ClientError('Missing id parameter for changeLog'));
