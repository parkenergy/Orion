      ...
      if (options.searchSupervisor) {
        User.getTechsForSupervisor(options.searchSupervisor)
          .then(techs => {
            ...
            log.trace({techs: techs}, "Admin Search Techs for Supervisor");
            options.supervised = techs;
            return WorkOrder._count(options);
          })
          .then(result => {
...
WorkOrderCtrl.update = (req, res) => {
  ...
  if ((!body.managerApproved || !body.timeApproved) && identity.role !== "admin") {
    ...
    if ((!body.managerApproved || !body.timeApproved) && !body.netsuiteSyned) {
      // this happens if just updating, it will auto approve
      WorkOrder.approveAndUpdate(id, body, identity)
        .then(res.resolve)
        .catch(res.reject);
    } else if ((body.managerApproved && body.timeApproved) && !body.netsuiteSyned) {
      // already updated and approved but just making another update
      WorkOrder.updateDoc(id, body, identity)
        .then(res.resolve)
        .catch(res.reject);
    } else if ((!body.managerApproved && !body.timeApproved) && body.netsuiteSyned) {
      // This will happen if not approved it will approve and sync
      WorkOrder.approveAndUpdate(id, body, identity)
        .then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc, identity))
        .then(res.resolve)
        .catch(res.reject);
    } else {
      // if already approved netsuite sync.
      WorkOrder.updateDoc(id, body, identity)
        .then(res.resolve)
        .catch(res.reject);
    }
...
WorkOrderCtrl.woDump = (req, res) => {
  ...
  log.trace({identity: identity}, "Indentified as");
  if(!identity) return res.reject(new AuthError("Not authenticated"));
  if(identity.role !== 'admin') return res.reject(new AuthError("Not Authorized"));
  ...
  try {
    ...
    if (options.searchSupervisor) {
      User.getTechsForSupervisor(options.searchSupervisor)
        .then((techs) => {
          if (!techs) techs = [options.searchSupervisor];
          log.trace({techs: techs}, "Admin Search for Techs for Supervisor report");
          options.supervised = techs;
          return WorkOrder.woDumpReport(options);
        })
        .then((result) => {
          res.downloadable("woDump.csv", "text/csv", result);
        }).catch(res.reject)
    } else {
      WorkOrder.woDumpReport(options)
        .then(result => {
          res.downloadable("woDump.csv", "text/csv", result);
        }).catch(res.reject)
    }
  ...
  try {
    ...
    if (options.searchSupervisor) {
      User.getTechsForSupervisor(options.searchSupervisor)
        ...
        .then((result) => {
          res.downloadable("woPartsDump.csv", "text/csv", result);
        }).catch(res.reject)
    } else {
      WorkOrder.woPartsDumpReport(options)
        .then(result => {
          res.downloadable("woPartsDump.csv", "text/csv", result);
        }).catch(res.reject)
    }
...
const Options = (params, role) => {
  const sorting = {
    ...
    approved:     params.approved   || false,
    synced:       params.synced     || false,
    searchSupervisor: params.searchSupervisor || false,
