  ...
  if ((!body.managerApproved || !body.timeApproved) && identity.role !== "admin") {
    ...
    if ((!body.managerApprove || !body.timeApproved) && !body.netsuiteSyned) {
      WorkOrder.approveAndUpdate(id, body, identity)
        .then(res.resolve)
        .catch(res.reject);
    } else if ((!body.managerApprove && !body.timeApproved) && body.netsuiteSyned) {
      // if if the work order has not been approved and an admin submits the
      // wo it is first approved then synced to netsuite
      WorkOrder.approveAndUpdate(id, body, identity)
