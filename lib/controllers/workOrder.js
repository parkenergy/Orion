...
* */
WorkOrderCtrl.adminCreate = (req, res) => {
    ...
    if (identity.role !== 'admin') return res.reject(new AuthError('You do not have permissions to do that'))

    WorkOrder.createAdminDoc(body)
        .then(res.resolve)
        .catch(res.reject)
}
    ...
    try {
        ...
        if (options.searchSupervisor) {
            User.getTechsForSupervisor(options.searchSupervisor).
                ...
                then((result) => {
                    res.downloadable('payrollDump.csv', 'text/csv', result);
                }).
                catch(res.reject);
        } else {
