...
WorkOrderCtrl.update = (req, res) => {
    ...
    ReviewNote.list({workOrder: wo})
        .then((notes) => {
             ...
             * if () {
             */
            if ((!body.managerApproved || !body.timeApproved) && identity.role === 'manager') {
                log.error('User update in manager section')
                if (canSync) {
                ...
                log.error('User update admin section')
                /**
                 * ------------------- sec: 2 (Update Only)-------------
                 * ("admin")
                 * ((!managerApproved || !timeApproved) && !netsuiteSyned)
                ...
                if ((!body.managerApproved || !body.timeApproved) && !body.netsuiteSyned) {
                    log.error('!manager !netsuiteSyned -> Update and approve')
                    ...
                    log.error('yesManager !netsuiteSyned -> just update')
                    ...
                    log.error('!manager yesNetsuiteSyned')
                    return WorkOrder.UpdateToSync(id, body, identity)
                        .then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc, identity))
                    ...
                    log.error('yesManager yesNetsuiteSyned')
                    return WorkOrder.UpdateToSync(id, body, identity)
                        .then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc, identity))
                    /**
                     * ------------------- sec: 6 ------------------------
