...
WorkOrderCtrl.update = (req, res) => {
    ...
    ReviewNote.list({workOrder: wo}).then((notes) => {
        ...
        if ((!body.managerApproved || !body.timeApproved) && identity.role ===
            ...
            if (canSync) {
                log.error(
                    'CTRL (259) fn:update !managerApproved canSync ->fn:UpdateToSync->fn:updateDoc');
                return WorkOrder.UpdateToSync(id, body, identity)
                    .then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc,
                        identity));
            } else {
                log.error(
                    'CTRL (263) fn:update !managerApproved !canSync ->fn:managerApprove');
            ...
            if ((!body.managerApproved || !body.timeApproved) &&
                !body.netsuiteSyned) {
                log.error(
                    'CTRL (287) fn:update !manager !netsuiteSyned ->fn:simpleUpdateAndApprove');
                ...
                !body.netsuiteSyned) {
                log.error(
                    'CTRL (301) fn:update Manager !netsuiteSyned ->fn:updateDoc');
                ...
                body.netsuiteSyned) {
                log.error(
                    'CTRL (315) fn:update !manager NetsuiteSyned ->fn:UpdateToSync->fn:updateDoc');
                return WorkOrder.UpdateToSync(id, body, identity)
                    .then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc,
                        identity));
                ...
                body.netsuiteSyned) {
                log.error(
                    'CTRL (330) fn:update ManagerApproved NetsuiteSyned ->fn:UpdateToSync->fn:updateDoc');
                return WorkOrder.UpdateToSync(id, body, identity)
                    .then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc,
                        identity));
                /**
