...
WorkOrderCtrl.create = (req, res) => {
    ...
    WorkOrder.createDoc(req.body).then(res.resolve).catch(res.reject);
};
            ...
            if (options.searchSupervisor) {
                User.getTechsForSupervisor(options.searchSupervisor).
                    then((techs) => {
                        if (!techs) techs = [options.searchSupervisor];
                        log.trace({techs: techs},
                            'Admin Search for Techs for Supervisor');
                        options.supervised = techs;
...
WorkOrderCtrl.update = (req, res) => {
    ...
    log.error('CTRL UPDATE!!!!! (233)');
    /**
     * Get all Review Notes for this WO. If none then canSync = true;
     */
    ReviewNote.list({workOrder: wo}).then((notes) => {
        let canSync = true;
        /**
         ...
         */
        notes.forEach((note) => {
            if (note.user === identity.username && identity.role ===
                'manager') {
                canSync = false;
            }
        });
        ...
        if ((!body.managerApproved || !body.timeApproved) && identity.role ===
            'manager') {
            log.error('CTRL (262) fn:update [MANAGER]');
            if (canSync) {
                log.error(
                    'CTRL (264) fn:update !managerApproved canSync ->fn:UpdateToSync->fn:updateDoc');
                return WorkOrder.UpdateToSync(id, body, identity).
                    then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc,
                        identity));
            } else {
                log.error(
                    'CTRL (268) fn:update !managerApproved !canSync ->fn:managerApprove');
            ...
            log.error('CTRL (280) fn:update [ADMIN]');
            /**
             * ------------------- sec: 2 (Update Only)-------------
             * ("admin")
             * ((!managerApproved || !timeApproved) && !netsuiteSyned)
            ...
            if ((!body.managerApproved || !body.timeApproved) &&
                !body.netsuiteSyned) {
                log.error(
                    'CTRL (291) fn:update !manager !netsuiteSyned ->fn:simpleUpdateAndApprove');
                ...
                !body.netsuiteSyned) {
                log.error(
                    'CTRL (304) fn:update Manager !netsuiteSyned ->fn:updateDoc');
                ...
                body.netsuiteSyned) {
                log.error(
                    'CTRL (317) fn:update !manager NetsuiteSyned ->fn:UpdateToSync->fn:updateDoc');
                return WorkOrder.UpdateToSync(id, body, identity).
                    then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc,
                        identity));
                ...
                body.netsuiteSyned) {
                log.error(
                    'CTRL (331) fn:update ManagerApproved NetsuiteSyned ->fn:UpdateToSync->fn:updateDoc');
                return WorkOrder.UpdateToSync(id, body, identity).
                    then((updatedDoc) => WorkOrder.updateDoc(id, updatedDoc,
                        identity));
                /**
                ...
                log.error('CTRL (343) fn:update else ->fn:updateDoc');
                return WorkOrder.updateDoc(id, body, identity);
...
WorkOrderCtrl.unapprovedByAreaM = (req, res) => {
    ...
    WorkOrder.unapprovedByArea(users).then(res.resolve).catch(res.reject);
};
...
WorkOrderCtrl.payrollDump = (req, res) => {
    ...
    try {
        ...
        if (options.searchSupervisor) {
            ...
            WorkOrder.woPayrollDump(options).then(result => {
                res.downloadable('payrollDump.tsv', 'text/tsv', result);
            }).catch(res.reject);
        }
    } catch (e) {
