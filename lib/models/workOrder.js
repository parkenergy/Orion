...
workOrderSchema.virtual('totalWoTime')
  .get(function(){
    //log.trace({laborCodes: this.laborCodes}, "totalWoTime, laborcodes");
    if(!this.timeSubmitted || !this.timeStarted) return {hours: 0, minutes: 0};
    var hrs = 0, nHrs = 0;
    var min = 0, nMin = 0;

    _.map(this.laborCodes, function(jsParentD){
      _.forEach(jsParentD,function(child,jsParentM){
        if(jsParentM === "negativeAdj"){
          nHrs = _.pick(child,['hours']);
          nMin = _.pick(child,['minutes']);
        }else{
          hrs = hrs + parseInt(_.pick(child,['hours']));
          min = min + parseInt(_.pick(child,['minutes']));
        }
      })
    })
    log.trace({totalHours: hrs, totalMin: min}, "Total Time");
    /*
    _.map(Object, function);
    _.pick(object,[hours,minutes]); -> {hours: #, minutes: #}
