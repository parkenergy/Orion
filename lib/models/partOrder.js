...
function updateTimeStamps(next) {
  ...
  this.timeOrdered = ( this.status === 'ordered' ) ? new Date() : null;
...
partOrderSchema.pre('findOneAndUpdate', function (next) {
  if(!this._update.timeOrdered){
    this._update.timeOrdered = (this._update.status === 'ordered') ? new Date() : null;
  }
...
partOrderSchema.pre('update', function (next) {
  ...
  this._update.$set.timeOrdered = ( this._update.$set.status === 'ordered' ) ? new Date() : null;
