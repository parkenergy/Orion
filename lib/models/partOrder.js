...
const partOrderSchema = new mongoose.Schema({
    ...
    partNSID: {type: Number, required: false},
    part: {
        vendor: {type: String},
        number: {type: String},
...
function updateTimeStamps(next) {
    this.timeSubmitted = (!this.timeSubmitted)
        ? new Date()
        : this.timeSubmitted;
    this.timeOrdered = (this.status === 'ordered') ? (!this.timeOrdered
        ? new Date()
        : this.timeOrdered) : null;
    this.timeComplete = (this.status === 'completed') ? (!this.timeComplete
        ? new Date()
...
partOrderSchema.pre('findOneAndUpdate', function (next) {
    if (!this._update.timeOrdered) {
        this._update.timeOrdered = (this._update.status === 'ordered')
            ? (!this._update.timeOrdered
                ? new Date()
                : this._update.timeOrdered)
            : null;
    }
    ...
    if (!this._update.timeComplete) {
        this._update.timeComplete = (this._update.status === 'completed')
            ? (!this._update.timeComplete
                ? new Date()
                : this._update.timeComplete)
            : null;
    }
...
partOrderSchema.pre('update', function (next) {
    ...
    this._update.$set.timeOrdered = (this._update.$set.status === 'ordered')
        ? (!this._update.$set.timeOrdered
            ? new Date()
            : this._update.$set.timeOrdered)
        : null;
    this._update.$set.timeComplete = (this._update.$set.status === 'completed')
        ? (!this._update.$set.timeComplete
            ? new Date()
            : this._update.$set.timeComplete)
        : null;
...
partOrderSchema.pre('findOneAndUpdate', function (next) {
    if (this._update.status === 'canceled' &&
        this._update.completedBy) this._update.done = true;
