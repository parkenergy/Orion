...
partOrderSchema.pre('findOneAndUpdate', function (next) {
  if(!this._update.timeShipped){
    this._update.timeShipped = (this._update.status === 'shipped') ? new Date() : null;
  }

  if(!this._update.timeComplete){
    this._update.timeComplete = (this._update.status === 'completed') ? new Date() : null;

    // if made on Orion
    if(this._update.status === 'canceled' && this._update.techId === this._update.approvedBy){
...
partOrderSchema.pre('update', function (next) {
  ...
  this._update.$set.timeShipped = ( this._update.$set.status === 'shipped' ) ? new Date() : null;
  this._update.$set.timeComplete = ( this._update.$set.status === 'completed') ? new Date() : null;
...
partOrderSchema.pre('findOneAndUpdate', function (next) {
  ...
  if(this._update.status === 'canceled' && this._update.techId === this._update.approvedBy){
    ...
    this._update.completedBy = this._update.approvedBy;
  }
  next();
});
