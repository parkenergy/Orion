...
class WOSync {
    ...
    constructor (WorkOrders, data, newArray) {
        ...
        // put synced wos that did sync
        this.synced = []
        // place wos that did not sync here
        this.notSynced = []
        // create WO/Sync
                    ...
                    workOrderLink.postRequest(nswo,
                        (body) => {
                            if (!isEmpty(body) && !isEmpty(body.error)) {
                                ...
                                log.error(`Doc could not reSync ${woDoc._id}`)
                            } else {
                                if (!body.nswoid) {
                                    woDoc.timeSynced = null
                                    woDoc.syncedBy = ''
                ...
                mailer.transport.verify(function (error, success) {
                    if (error) {
                        ...
                        let mailOptions = {
                            ...
                            to:      to,
                            subject: `Synced WOs from Auto sync. # Synced: ${this.synced.length}, not Synced: ${this.notSynced.length}`,
                            html: `
                            <body>
                                ${(() => {
                                    this.synced.reduce((acc, cur) => {
                                        if (cur) {
