...
class WOSync {
    ...
    constructor (WorkOrders, data, newArray) {
        this.url = 'https://rest.na1.netsuite.com/app/site/hosting/restlet.nl?script=112&deploy=1'
        this.WorkOrders = WorkOrders
        this.data = data
        this.newArray = newArray
        // create WO/Sync
        this.woCreateGetContentChecksum = this.woCreateGetContentChecksum.bind(this)
        ...
        this.setCustomerAndFormatOnInsert = this.setCustomerAndFormatOnInsert.bind(this)
        ...
        try {
            return {
                ...
                compDiffPCFilterKill:  wo.unitReadings.compDiffPCFilterKill
                                           ...
                                           : '', lubeRate: wo.unitReadings.lubeRate ? wo.unitReadings.lubeRate : '',
                ...
                testPInchesH2O:          wo.emissionsReadings.testPInchesH2O
                                             ...
                                             : '',
                // spotCheck:               wo.emissionsReadings.spotCheck === undefined ? false : wo.emissionsReadings.spotCheck,
                // noSpotCheck:             wo.emissionsReadings.noSpotCheck === undefined ? false : wo.emissionsReadings.noSpotCheck,
                // lastCalibration:         wo.emissionsReadings.lastCalibration ? wo.emissionsReadings.lastCalibration : '',
                // NOxGrams:                wo.emissionsReadings.NOxGrams ? wo.emissionsReadings.NOxGrams : '',
                // COGrams:                 wo.emissionsReadings.COGrams ? wo.emissionsReadings.COGrams : '',
                // NOxAllowable:            wo.emissionsReadings.NOxAllowable ? wo.emissionsReadings.NOxAllowable : '',
                // COAllowable:             wo.emissionsReadings.COAllowable ? wo.emissionsReadings.COAllowable : '',

                spotCheck:               wo.emissionsReadings.spotCheck === undefined ? false : wo.emissionsReadings.spotCheck,
                noSpotCheck:             wo.emissionsReadings.noSpotCheck === undefined ? false : wo.emissionsReadings.noSpotCheck,
                lastCalibration: wo.emissionsReadings.lastCalibration ? wo.emissionsReadings.lastCalibration : null,
                NOxGrams:                wo.emissionsReadings.NOxGrams ? wo.emissionsReadings.NOxGrams : '',
                COGrams:                 wo.emissionsReadings.COGrams ? wo.emissionsReadings.COGrams : '',
                NOxAllowable:            wo.emissionsReadings.NOxAllowable ? wo.emissionsReadings.NOxAllowable : '',
                COAllowable:             wo.emissionsReadings.COAllowable ? wo.emissionsReadings.COAllowable : '',
            ...
            let toSync = toSyncObjects.reduce((acc, cur) => {
                if (cur.sync) {
                    ...
                    if (process.env.NODE_ENV === undefined ||
                        ...
                        return acc.concat({returnOnly: false, sync: cur.sync, doc: syncDoc})
                    }
                } else {
                    return acc
                }
        ...
        return objs.reduce((acc, cur) => {
            if (cur.sync) {
                return acc.concat({
                    ...
                    synced: false,
                    NSReturnObj: null,
                    cb: new TBA('https://rest.na1.netsuite.com/app/site/hosting/restlet.nl?script=112&deploy=1') // postRequest
                })
            } else {
                return acc.concat({
                    returnOnly: cur.returnOnly,
        ...
        return new Promise((resolve, reject) => {
            if (woDocInfo.sync) {
                WOSync.populateCustomerByName(woDocInfo.doc)
                      ...
                      .then((res) => {
                          resolve({returnOnly: woDocInfo.returnOnly, nswo: res, woDoc: woDocInfo.doc, sync: woDocInfo.sync})
                      })
                      .catch(reject)
            } else {
                this.WorkOrders.find({
                    ...
                    .then((docs) => {
                        if (docs.length > 0) {
                            const doc = docs[0]
                            return this.WorkOrders.findByIdAndUpdate(doc._id, woDocInfo.doc, {safe: false, new: true})
                                .exec()
                        } else {
                    ...
                    .then((res) => {
                        resolve({returnOnly: woDocInfo.returnOnly, nswo: null, woDoc: res, sync: woDocInfo.sync})
                    })
                    .catch(reject)
            }
        })
        ...
        return new Promise((resolve, reject) => {
            ...
            WOSync.populateCustomerByName(woDoc.doc)
                  ...
                  .then((res) => {
                      ...
                      resolve({returnOnly: woDoc.returnOnly, nswo: res, woDoc: woDoc.doc, sync: woDoc.sync})
                  })
                  .catch((err) => {
        ...
        mailer.transport.verify(function (error, success) {
            if (error) {
                ...
                let mailOptions = {
                    ...
                    html: `
                    <body>
                        <a href="http://orion.parkenergyservices.com/#/workorder/review/${wo._id}">WorkOrder:" +
                        " ${wo._id} |" +
                ...
                mailer.transport.sendMail(mailOptions, (error, info) => {
                    if (error) {
                        log.error({error: error}, 'Error while attempty to send error message and workorder to orion' +
    ...
    ReTryAutoSyncAllToNetsuite(docs, index) {
        return new Promise((resolve, reject) => {
           ...
           if (!obj.returnOnly) {
               obj.cb.postRequest(obj.formattedWO, (body) => {
                   ...
                   if (!isEmpty(body) && !isEmpty(body.error)) {
                       ...
                       if (['ECONNRESET', 'ESOCKETTIMEDOUT', 'ETIMEDOUT', 'SSS_REQUEST_LIMIT_EXCEEDED'].indexOf(error_message) !==
                       ...
                       } else if (['INVALID_KEY_OR_REF'].indexOf(error_message) !== -1) {
                           // error with a key .. Email out to orion alerts the error
                           log.error(`Work order sync error, received non limit exceeded body1: ${obj.unformattedWO._id} | ${error_message}`)
                           this.emailOrionAlertsError(error_message, obj.unformattedWO)
                           WOSync.resyncNext(docs, index, this.ReTryAutoSyncAllToNetsuite)
                                 .then(resolve)
                                 .catch(reject)
                       } else {
                           ...
                           // next
                           obj = WOSync.woDidNotSync(obj)
                           log.error(`Work order did not sync, received non limit exceeded body2: ${obj.unformattedWO._id} | ${error_message}`)
                           obj.repeats = 0
                   ...
                   if (['ECONNRESET', 'ESOCKETTIMEDOUT', 'ETIMEDOUT', 'SSS_REQUEST_LIMIT_EXCEEDED'].indexOf(error_message) !==
                       ...
                       // error with a key .. Email out to orion alerts the error
                       log.error(`Work order sync error, received non limit exceeded error1: ${obj.unformattedWO._id} | ${error_message}`)
                       this.emailOrionAlertsError(error_message, obj.unformattedWO)
                       WOSync.resyncNext(docs, index, this.ReTryAutoSyncAllToNetsuite)
                             .then(resolve)
                             .catch(reject)
                       ...
                       obj = WOSync.woDidNotSync(obj)
                       obj.repeats = 0
                       log.error(`Work order sync error, received non limit exceeded error2: ${obj.unformattedWO._id} | ${error_message}`)
                       // go to next on this error type
                       WOSync.resyncNext(docs, index, this.ReTryAutoSyncAllToNetsuite)
                             .then(resolve)
                             .catch(reject)
            ...
            this.newArray.forEach((item) => {
                if (!item.found) {
                    ...
                    toSyncObjects.forEach((wo) => {
                        if ((wo.timeSubmitted.toString() === new Date(item.obj.timeSubmitted).toString()) && (item.found === false)) {
                            if (item.sync && item.obj.type !== 'Indirect') {
                                ...
                                /*toSync.push({returnOnly: false, sync: true, doc: SyncDoc})*/
                                if (process.env.NODE_ENV !== undefined && process.env.NODE_ENV !== 'test' && process.env.NODE_ENV !== 'development') {
                                    toSync.push({returnOnly: false, sync: true, doc: SyncDoc})
                                } else {
                                    toSync.push({returnOnly: true, sync: false, doc: SyncDoc})
                                }
                            } else if (item.managerSync && item.obj.type !== 'Indirect') {
                                ...
                                toSync.push({returnOnly: true, sync: false, doc: updateDoc})
                            } else {
                                toSync.push({returnOnly: true, sync: false, doc: item.obj})
                            }
                        }
                    })
                } else {
    ...
    static syncedLinks(synced) {
        return synced.reduce((acc, cur) => {
            if (cur) {
                return acc.concat(`<a href="http://orion.parkenergyservices.com/#/workorder/review/${cur._id}">Synced WO ${cur.type} | ${cur.header.unitNumber} | ${cur.techId} | ${cur.header.customerName} | ${cur.header.leaseName}</a>
                                            <br>`)
            } else {
                return acc
            }
    ...
    static notSynced(notSynced) {
        return notSynced.reduce((acc, cur) => {
            if (cur) {
                return acc.concat(`<a href="http://orion.parkenergyservices.com/#/workorder/review/${cur._id}">not Synced WO ${cur.type} | ${cur.header.unitNumber} | ${cur.techId} | ${cur.header.customerName} | ${cur.header.leaseName}</a>
                                            <br>`)
            } else {
                return acc
            }
