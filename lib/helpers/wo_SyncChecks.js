...
class WOSync {
    ...
    setUpWOsForSync (toSyncObjects) {
        return new Promise((resolve, reject) => {
            ...
            let items = TH.processArray(toSync, this.setCustomerAndFormat)
            resolve(items)
        })
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (woDocInfo.sync) {
                WOSync.populateCustomerByName(woDocInfo.doc)
                      ...
                      .then((res) => {
                          resolve({nswo: res, woDoc: woDocInfo.doc, sync: woDocInfo.sync})
                      })
                      .catch(reject)
            } else {
                this.WorkOrders.find({
                    ...
                    .then((res) => {
                        resolve({nswo: null, woDoc: woDocInfo.doc, sync: res})
                    })
                    .catch(reject)
            }
        })
    ...
    setUpInsertedWOsForSync(toSyncObjects) {
        return new Promise((resolve, reject) => {
                        ...
                        if ((wo.timeSubmitted.toString() === new Date(item.obj.timeSubmitted).toString()) && (item.found === false)) {
                            if (item.sync) {
                                if (process.env.NODE_ENV !== undefined && process.env.NODE_ENV !== 'test' && process.env.NODE_ENV !== 'development') {
                                    ...
                                    toSync.push({sync: true, doc: SyncDoc})
                                }
                                ...
                                toSync.push({sync: false, doc: updateDoc})
                            }
                        }
                    })
                }
            ...
            let items = TH.processArray(toSync, this.setCustomerAndFormatOnInsert)
            resolve(items)
        })
    }
     ...
     * @param toSyncDocumentObject -> wos
     * @returns {*}
     */
