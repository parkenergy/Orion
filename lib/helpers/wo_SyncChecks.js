...
class WOSync {
    ...
    static async populateCustomerByName (wo) {
        try {
            const cust = await Customers.findOne({name: wo.header.customerName}).lean().exec()
            log.info({customer: cust}, 'Customer Found')
            wo.customer = cust || null
            if (wo.customer === null) {
                wo.customer = {netsuiteId: ''}
    ...
    netsuiteSyncTrys(repeats, retries, backoff, nswo, woDoc) {
        try {
            const workOrderLink = new TBA('https://rest.na1.netsuite.com/app/site/hosting/restlet.nl?script=112&deploy=1')
            workOrderLink.postRequest(nswo,
                (body) => {
                            ...
                            if (repeats > 0) {
                                ...
                                if (backoff && retries) {
                                    ...
                                    setTimeout(() => {
                                        ...
                                        this.netsuiteSyncTrys((repeats - 1), retries, backoff, nswo, woDoc)
                                    }, backoff * (retries - (repeats + 1)))
                                } else {
    ...
    async ResyncToNetsuite(woDoc) {
        try {
            ...
            const wo = await WOSync.populateCustomerByName(woDoc)
            const nswo = WOSync.netSuiteFormat(wo)
            log.info('returned from customer and netsuite format')
            let backoff = 200
            let retries = 30
            return await this.netsuiteSyncTrys(retries, retries, backoff, nswo, woDoc)
        } catch (err) {
            throw Error(err)
        }
    }
    ...
    /**
     * Method takes in all workorders that have not synced and syncs them
     * if they need to be
    ...
    tryReSyncWOs (toSyncObjects) {
        return new Promise((resolve, reject) => {
            ...
            let items = processArray(toSync, this.ReTryAutoSyncToNetsuite)
            resolve(items)
        })
    }
     ...
     * @constructor
     */
    async ReTryAutoSyncToNetsuite (doc) {
        try {
            const syncedDoc = await this.ResyncToNetsuite(doc)
            return await this.updateDocAfterSync(syncedDoc)
        } catch (err) {
    ...
    syncedLinks(synced) {
        return synced.reduce((acc, cur) => {
            if (cur) {
                return acc.concat(`<a href="http://orion.parkenergyservices.com/#/workorder/review/${cur}">Synced WO ${cur}</a>
                                            <br>`)
            } else {
                return acc
            }
    ...
    notSynced(notSynced) {
        return notSynced.reduce((acc, cur) => {
            if (cur) {
                return acc.concat(`<a href="http://orion.parkenergyservices.com/#/workorder/review/${cur}">not Synced WO ${cur}</a>
                                            <br>`)
            } else {
                return acc
            }
