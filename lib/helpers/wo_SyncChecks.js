...
class WOSync {
    ...
    static syncToNetsuite (woDoc) {
        return new Promise((resolve, reject) => {
            console.log('Sync unitReadings: ', JSON.stringify(woDoc.unitReadings, null, 2))
                   ...
                   if (!isEmpty(body) && !isEmpty(body.error)) {
                       ...
                       if (['ECONNRESET', 'ESOCKETTIMEDOUT', 'ETIMEDOUT', 'SSS_REQUEST_LIMIT_EXCEEDED'].indexOf(error_message) !==
                           ...
                           if (obj.repeats > 0) {
                               ...
                               log.error(`Work order did not sync: ${obj.unformattedWO._id} ${JSON.stringify(body)}`)
                               obj.repeats = 0
                           ...
                           log.error(`Work order sync error, received non limit exceeded body1: ${obj.unformattedWO._id} | ${JSON.stringify(body)}`)
                           ...
                           // next
                           obj = WOSync.woDidNotSync(obj)
                           log.error(`Work order did not sync, received non limit exceeded body2: ${obj.unformattedWO._id} | ${JSON.stringify(body)}`)
                           obj.repeats = 0
    ...
    emailOrionAlertsError(error_message, wo) {
        ...
        let to
        if (process.env.NODE_ENV === undefined || process.env.NODE_ENV === 'development' || process.env.NODE_ENV ===
            'test') {
