...
const taskHelper = {
  ...
  sixAm(b) {
    const To = new Date(b);
    return new Date(To.setHours(6, 0, 0, 0));
  },
  ...
  previousWeekEndDateAndCheckDate() {
    ...
    if (now.getDay() === 0) {
      checkDay = this.sixAm(new Date(now.setDate(now.getDate() + (1 - now.getDay()))));
    } else if (now.getDay() === 6) {
      checkDay = this.sixAm(new Date(now.setDate(now.getDate() - (now.getDay() - 8))));
    } else {
      checkDay = this.sixAm(new Date(now.setDate(now.getDate() - (now.getDay() - 1))));
    }
