    ...
    addChangeLogUser (docs, users) {
        return new Promise((resolve, reject) => {
            ...
            users.forEach((user) => {
                ...
                docs.forEach((doc) => {
                    if (doc.netsuiteId === user.netsuiteId) {
                        found = true;
                        changes.push(
                            {diff: diff(user, doc), old: user.netsuiteId, newDoc: doc.netsuiteId});
                    }
                });
                if (!found) {
            ...
            changes.forEach((change) => {
                ...
                if (change.diff !== undefined && change.diff !== 'removed' && change.diff !==
                    'added') {
        ...
        for (let i = 0; i < days; i++) {
            if (i === 0) {
                ...
                loopDays.push({
                    ...
                    end:   this.endOfDay(new Date(DateFrom + (oneDay * i))).getTime(),
                });
            }
        }
        return loopDays;
            ...
            try {
                ...
                laborCodes.forEach((lb) => {
                    ...
                    childLaborCodeKeys.forEach((chlb) => {
                        if (chlb !== '$init' && chlb !== 'positiveAdj' &&
                            chlb !== 'negativeAdj' && chlb !== 'lunch' &&
                            wo.laborCodes[lb][chlb].hasOwnProperty('hours')) {
                            Total.minutes += +wo.laborCodes[lb][chlb].hours *
                                60;
    ...
    getTotalWOTimeNoPromise (wo) {
        try {
            ...
            laborCodes.forEach((lb) => {
                const childLaborCodeKeys = Object.keys(wo.laborCodes[lb]);
                childLaborCodeKeys.forEach((chlb) => {
                    if (chlb !== '$init' && chlb !== 'positiveAdj' && chlb !==
                        'negativeAdj' && chlb !== 'lunch') {
                        if (wo.laborCodes[lb][chlb] && wo.laborCodes[lb][chlb].hours > -1 &&
            ...
            Total.time = `${this.pad(
                Math.floor(Total.totalMin / 60))}:${this.pad(
                Math.round(Total.totalMin % 60))}`;
            return Total;
        } catch (e) {
            throw new Error(e);
        }
        ...
        return new Promise((resolve, reject) => {
            WorkOrder.find({
                ...
                .then((objArr) => {
                    if (objArr.length === 1) {
                        resolve(
                            {user, date: date, Total: objArr[0].Total.hours});
                    } else {
        ...
        return new Promise((resolve, reject) => {
            PaidTimeOff.find({
                ...
                .then((ptos) => {
                    if (ptos.length === 0) {
                        resolve(
                            {user: obj.user, date: obj.date, Total: obj.Total});
                    } else {
    ...
    addTechsToSupervisor (SuperObj, nonTechUsername) {
        return new Promise((resolve, reject) => {
            try {
                ...
                const supervisorSuper = SuperObj[nonTechUsername].SupervisorObj.supervisor;
                ...
                if (supervisorSuper) {
                    ...
                    SuperObj[supervisorSuper].manages[nonTechUsername] = SuperObj[nonTechUsername].SupervisorObj;
                    // do the same for OrionAlerts
                    SuperObj['Orion'].manages[nonTechUsername] = SuperObj[nonTechUsername].SupervisorObj;
