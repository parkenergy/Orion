...
const taskHelper = {
    ...
    getTotalLaborCodeTime (wo) {
        return new Promise((resolve, reject) => {
            try {
                ...
                const laborCodes = Object.keys(wo.laborCodes); // basic,
                                                               // engine, ...
                                                               // so on
                laborCodes.forEach((lb) => {
                    const childLaborCodeKeys = Object.keys(wo.laborCodes[lb]); // basic:
                                                                               // {
                                                                               // safety...
                                                                               // so
                                                                               // on
    ...
    getTotalWOTimeNoPromise (wo) {
        try {
            ...
            const laborCodes = Object.keys(wo.laborCodes); // basic, engine,
                                                           // ... so on
            laborCodes.forEach((lb) => {
                const childLaborCodeKeys = Object.keys(wo.laborCodes[lb]); // basic:
                                                                           // {
    ...
    addTechsToSupervisor (superObj, supervisor) {
        return new Promise((resolve, reject) => {
            try {
                ...
                if (supervisorSuper) {
                    const techs = Object.keys(superObj[supervisor].techs); // all
                                                                           // usernames
                                                                           // of
                                                                           // techs
                                                                           // for
