      ...
      .then((docs) => {
        return docs.reduce((csv, row) => {
          ...
          if (row.typeofitem === "workorder") {
            ...
            var te = new Date(timeStarted.getHours() + row.totalWOTimebase10); // timeEnd =  start time plus laborcodes - adjustments

            if (ts1.getDay() != te.getDay()) {
              var ts2 = new Date(ts2.getYear(), ts2.getMonth(), ts2.getDay()); //   0000 o'clock second day of workorder
              console.info(
                "after Midnight " +
                  tst +
                  " " +
