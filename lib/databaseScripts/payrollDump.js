    ...
    cursor
      ...
      .then((docs) => {
        ...
        //    var te = new Date(timeStarted);
         //   te.setHours(te.getHours() + row.totalWOTimebase10); // timeEnd =  start time plus laborcodes - adjustments
              var te = new Date(ts1.getTime() + row.totalWOTimebase10*60*60*1000);

            if (ts1.getDay() != wte.getDay()) {
              var ts2 = new Date(te.getFullYear(), te.getMonth(), te.getDay()); //   0000 o'clock second day of workorder
              ...
              csv +=
                [
                  ...
                  TH.sanitize(row.comments.indirectNotes), "ts1= "+ts1+"         wte="+wte+"      ts2 = " +ts2+"    te="+te,
                ].join(",") + "\n";
              csv +=
                [
                  ts2,
