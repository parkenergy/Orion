...
//
    ...
    //  query.timeStarted.$gte = new Date( "2021-20-31T05:00:00Z");  
     // console.log(query.timeStarted.$gte) ;
            ...
            qlte = qlte.toLocaleString('en-US', { timeZone: 'America/Chicago' });
      var tdiff = query.timeStarted.$lte - qlte; 
            var qedate = new Date(qlte);
...
//Start date from UTC to OKlahoma time
        ...
        }, "TimeStarted,TimeSubmitted,TechId,TruckId,Total Hours Decimal,Type,AtShop,Header.unitNumber,Header.customer,Header.county,Header.state,Header.lease,Comments.indirectNotes,"+ tdiff+",\n");
      })
      .then((r) => {
        resolve(r);
      })
