...
module.exports = (db, query, PTOs) => {
    return new Promise((resolve, reject) => {
        ...
        let ptos = [];
        let wos = [];
        const setPTOTimeForUser = (ptos, diff) => {
            let done = false;
            ptos.forEach((pto) => {
                if (!done) {
        ...
        const createPTOcsvObjs = (ptos, wos, days) => {
            return new Promise((resolve, reject) => {
                ...
                days.forEach((day) => {
                    ...
                    wos.forEach((wo) => {
                        if (isEmpty(users[wo.techId])) {
                            users[wo.techId] = {PTOs: [], WOs: [], hours: 0};
                        }
                ...
                USERS.forEach((user) => {
                    ...
                    if (users[user].PTOs.length > 0 && users[user].hours > 40) {
                        ...
                        if ((diff === users[user].ptoTime) || (diff > users[user].ptoTime)) {
                            ...
                            setPTOTimeForUser(users[user].PTOs, diff);
                        }
                    }
                });
                const returnPTOs = [];
        ...
        try {
            db.aggregate()
                ...
                .then((docs) => {
                    wos = docs;
                    return PTOs.find({
                        DateFrom: {$gte: query.timeStarted.$gte},
                    }).exec();
                })
                .then((docs) => {
                    ptos = ptos.concat(docs);
                    return PTOs.find({
                        DateTo: {$gte: query.timeStarted.$gte},
                    }).exec();
                })
                .then((docs) => {
