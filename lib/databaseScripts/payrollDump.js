...
//query.timeStarted.$gte = new Date ('2021-05-03T00:00:00'); 
    const returnPTO1 = () => {
      return new Promise((resolve, reject) => {
...
var StartdateValue = new Date(row.timeStarted);
var EnddateValue = new Date(row.timeSubmitted); 
// Get dates for January and July
dateJan = new Date(StartdateValue.getFullYear(), 0, 1);
dateJul = new Date(StartdateValue.getFullYear(), 6, 1);
// Get timezone offset
timezoneOffset = Math.max(dateJan.getTimezoneOffset(), dateJul.getTimezoneOffset());

// Check if daylight savings
if (StartdateValue.getTimezoneOffset() < timezoneOffset) {
  // Adjust date by 5 hours
  StartdateValue = new Date(StartdateValue.getTime() - ((1 * 60 * 60 * 1000) * 5));
    EnddateValue = new Date(EnddateValue.getTime() - ((1 * 60 * 60 * 1000) * 5));
}
else {
  // Adjust date by 6 hours
  StartdateValue = new Date(StartdateValue.getTime() - ((1 * 60 * 60 * 1000) * 6));
    EnddateValue = new Date(EnddateValue.getTime() - ((1 * 60 * 60 * 1000) * 6));
}
...
const offset = ts1.getTimezoneOffset();
        s= new Date(ts1.getTime() + offset * 60 * 1000);
                
              csv +=
                [
                  timeStarted + " " + timeStarted.toLocaleString('en-US', { timeZone: 'America/Chicago' });
                  timeSubmitted + " " + EnddateValue,
                  row.techId,
                 row.truckId,
                  hoursDay1,
                  row.type,
