    ...
    return new Promise((resolve, reject) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                days.forEach(day => {
                    ptos.forEach(pto => {
                        ...
                        pto.ptoDays.forEach(ptoDay => {
                            if (
                                new Date(ptoDay.dateOf).getTime() >=
                                    day.start &&
                        ...
                        if (pto.status !== "Rejected") {
                            ...
                            pto.ptoDays.forEach(ptoday => {
                                if (
                                    ...
                                    const dateFrom = TH.startOfDay(
                                        new Date(ptoday.dateOf)
                                    );
                                    const dateTo = TH.endOfDay(
                                        new Date(ptoday.dateOf)
                                    );
                                    returnPTOs.push({
        ...
        cursor
            ...
            .then(docs => {
                return docs.reduce((csv, row) => {
                   // console.info(row)
                    if (row.typeofitem === "workorder") {
                        const timeStarteda = TH.toExcelTime(row.timeStarted);
                        const timeSubmitted = TH.toExcelTime(row.timeSubmitted);
                        return (
                            csv +
                            [
                                timeStarteda + ' ' + row.timeStarted,
                                timeSubmitted,
                                row.techId,
                                row.truckId,
                ...
                }, "TimeStarted.UTC,TimeSubmitted.UTC,TechId,TruckId,Total Hours Decimal,Type,AtShop,Header.unitNumber,Header.customer,Header.county,Header.state,Header.lease,Comments.indirectNotes\n");
            })
            .then(r => {
                resolve(r);
            })
