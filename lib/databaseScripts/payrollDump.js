                ...
                return docs.reduce((csv, row) => {
                   // console.info(row)
                    if (row.typeofitem === "workorder") {
                        ...
                        var ts2 = new Date(timeSubmitted); 
                 if (ts1.getDay() != ts2.getDay()){
                    const offset = ts1.getTimezoneOffset();
         var tst = new Date(ts1.getTime() + offset * 60 * 1000);
   
                    console.info("after Midnight " + tst + " " + timeStarted + " to " + row.timeSubmitted + " " + timeSubmitted); 
                 }
                            return (
                            csv +
                            [
